...
- **Group By (file browser) — group files into collapsible sections.** New "View → Group Files By" menu with "None" (default) and "Kind". When grouping by kind, files are split into collapsible sections (Images, Documents, …); sections start expanded and you can collapse them. Built on the same `NSOutlineView` foundation as the hidden-files group. Both the file browser and the archive browser support it, each with its own "View" menu entry ("Group Files By" / "Group Archive By").
