...
- **The folder view now refreshes itself.** SimpleZip watches the folder you're viewing (via FSEvents) and updates the list automatically whenever its contents change — whether you added/removed/renamed something inside the app or another app did. No more stale listings or reaching for ⌘R after an external change — and your current selection is preserved across an auto-refresh. (Manual ⌘R still works.) File operations (paste, delete, rename, drag-in/out) no longer each refresh on their own — the watcher is the single source of refresh, so nothing slips through.
