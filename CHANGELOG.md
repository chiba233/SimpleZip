...
- **Fixed: launching SimpleZip by opening an archive from Finder showed no window.** When the app wasn't already running, double-clicking an archive launched it but left no window on screen — you had to click the Dock icon to make one appear. It now reliably opens a window for the file on a cold launch. (The fix keeps the safeguard that prevents `.siz` / `.szs` from spawning duplicate windows.)
