...
- **Faster extraction via multi-threading.** The 7-Zip backend now extracts with `-mmt=on`, letting it use all CPU cores for formats whose files decompress independently (e.g. ZIP) instead of staying single-threaded. (Extracting huge numbers of tiny files is still partly bound by the filesystem itself, not CPU.)
