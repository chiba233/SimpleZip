...
- **Fixed: rubber-band (marquee) selection flickered and convulsed.** The list was reloading on *every* selection change — so dragging a selection box reloaded the table under your cursor, fighting the drag. The list now reloads only when its actual content changes (files, sorting, grouping, columns, size), never on selection alone, so marquee-selecting is smooth.
