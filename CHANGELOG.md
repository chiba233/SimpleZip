...
- **Main window now supports native tabs (⌘T).** Press ⌘T for a new tab, and opening an archive or folder from Finder while SimpleZip is already running opens it in a new tab instead of replacing what you were looking at. These are real macOS window tabs — the tab bar auto-hides at one tab, and the Window menu's "Show Tab Bar" / "Move Tab to New Window" / "Merge All Windows" all work. Each tab is fully independent (its own folder, selection, and undo history). Settings, About, and the update window stay separate windows and never join the tab group.
