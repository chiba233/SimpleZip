...
> Apple 最多注册 **10 个 App Shortcut**(即带预置 Siri 短语的那些)。下面前十个动作是这样注册的;较新的动作
>(Compute File Hash 及各分析/发布工具)**未**预注册,所以没有内置 Siri 短语——但它们照常出现在「快捷指令」App 的
> SimpleZip 动作列表里,加进快捷指令后用法完全相同。
...
为一个或多个文件计算校验和(CRC32、MD5、SHA-1、SHA-256 或 SHA-512),按输入顺序返回十六进制摘要——可直接串到下一个动作。

| 参数 | 类型 | 说明 |
| --- | --- | --- |
| **Files** | 文件(多选) | 要计算哈希的文件。 |
| **Algorithm** | 选项 | `CRC32`、`MD5`、`SHA-1`、`SHA-256` 或 `SHA-512`。默认 `SHA-256`。 |

返回一组十六进制摘要字符串。

### Analyze Archive Space

返回归档的一句话体积占用摘要:原始 vs 压缩后大小、压缩率、macOS 垃圾,以及最大文件。只读。
...
从损坏归档尽力救援到旁边新建的 `(rescued)` 文件夹。救出的文件可能不完整,且**不**修复归档本身;救出的产物仍过不可信条目
安全检查。记入活动中心。
...
对多个归档逐个做完整性测试,只有全部通过才返回真。无人值守——条目名需要口令的归档算作未通过,而不弹框。

| 参数 | 类型 | 说明 |
| --- | --- | --- |
| **Archives** | 文件(多选) | 要体检的归档。 |

返回布尔值(全部通过为真)。

### Find Duplicate Archives

在给定归档中按结构指纹查找重复归档。每个重复组返回一行(成员文件名)。只读。

| 参数 | 类型 | 说明 |
| --- | --- | --- |
| **Archives** | 文件(多选) | 两个或更多要比较的归档。 |

返回重复组描述列表(没有则为空)。

### Check Reproducible Packing

用可复现设置把文件夹打包两次,两个归档逐字节一致时返回真。仅支持 zip 与 7z。临时产物写系统 temp、完事即清。

| 参数 | 类型 | 说明 |
| --- | --- | --- |
| **Folder** | 文件 | 要打包两次的源文件夹。须为目录。 |
| **Format** | 选项 | `ZIP` 或 `7-Zip`(其他格式会被拒绝)。默认 `ZIP`。 |

返回布尔值(两次构建逐字节一致为真)。

### Audit Release Directory

按文件名与校验文件审计发布目录(不算哈希):检查每个产物是否都被 SHA256SUMS 覆盖。覆盖完整时返回真。只读。

| 参数 | 类型 | 说明 |
| --- | --- | --- |
| **Folder** | 文件 | 要审计的发布目录。须为目录。 |

返回布尔值(每个产物都有校验为真)。

### Quick Verify Release Group

只按文件名快速核对发布目录的组成。下载者能据此校验(有产物或签名容器 + SHA256SUMS)时返回真。不读取任何内容。

| 参数 | 类型 | 说明 |
| --- | --- | --- |
| **Folder** | 文件 | 要核对的发布目录。须为目录。 |
