...
> ⚠️ 这份拆分计划目前**尚未执行**。自上次快照以来多数大文件继续增长、并新增了若干超大文件(下表已刷新到
> 当前行数);`FileTable` 的内联重命名已先抽到 `FileTableEditing.swift`,但主文件仍超 1800 行。拆法(P1–P4)
> 依然有效,恢复时按下面「建议节奏」逐刀来。

| 文件 | 行数 | 状态 |
|---|---:|---|
| Features/ArchiveBrowser/FileTable.swift | 1824 | 🔴 待拆(P1);FileTableEditing 已先抽出 |
| ArchiveBrowserModel+TestHashBenchmark.swift | 1774 | 🟡 新晋大文件(测试/哈希/benchmark 三职责) |
| ArchiveBrowserModel+CreateExtract.swift | 1622 | 🟡 待拆(P3) |
| App/ContentView.swift | 1543 | 🟡 已有任务 #87/#92 |
| ArchiveBrowserModel+FileOps.swift | 1457 | 🟡 已有任务 #101 |
| Features/Welcome/WelcomeAssistantView.swift | 1423 | 🟡 待拆(P4) |
| Features/Settings/Panes/GPGPane.swift | 1383 | 🟡 已有任务 #96 |
| Core/AppPreferences.swift | 1334 | 🟡 待拆(P2) |
| ArchiveCreationOptionsView.swift | 1228 | 🟢 低优先 |
| Features/ArchiveBrowser/ArchiveTable.swift | 875 | 🟢 已部分拆(ArchiveColumn 外移) |
| ArchiveExtractionCoordinator.swift | 847 | 🟢 0.3.0 已拆过一轮 |
