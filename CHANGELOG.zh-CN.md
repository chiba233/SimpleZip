...
- **创建与解压对话框新增实时 AI 速览（macOS 26）。** 打开任一对话框，当 Apple Intelligence 可用时，预检概要下方会自动出现一段 AI 速览——无需点按钮——给出大致耗时感受和一条实用提醒：创建时是格式/级别建议与同名冲突提醒；解压时则标出可能逃出目标文件夹的可疑路径、会被覆盖的文件、缺失分卷或磁盘空间不足。它会随你改格式、级别或目标位置实时更新，全程本地推理，只描述/建议，绝不改设置、绝不跳过安全询问。AI 助手关闭或不可用时整体隐藏。
