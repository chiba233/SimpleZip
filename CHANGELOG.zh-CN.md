...
- **新增:XIP 支持(浏览 / 解压 / 测试)。** `.xip`——Apple 的签名归档格式(Xcode 的分发方式)——现在能像普通压缩包一样打开:浏览即时列出容器成员,测试校验容器校验和,**整包解压走系统自带的 `xip` 工具**——拿到真实内容物(比如 `.app`),解压过程中由 macOS 校验 Apple 签名;系统不信任其签名的 `.xip` 会被拒绝并显示错误。解压*选中条目*则取出容器的原始成员(即浏览时看到的那些)。不提供创建 `.xip`(需要 Apple 证书)。已注册为可打开的文件类型、列入「格式能力」表,README 与指南同步更新。
