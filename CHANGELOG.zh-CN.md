...
- **新增:按格式的默认压缩设置。** 设置 → 压缩 → 默认压缩设置 里可以给每个格式(ZIP、7z、RAR、TAR.GZ…)配一份可复用的默认值——等级、7-Zip 方法 / 字典 / 固实 / 路径模式、排除规则、原始参数等,控件随所选格式自适应。Finder 的「压缩为 ZIP / 7z / TAR.GZ」一键服务(及对应 NSService)会自动套用对应格式的默认值。密码和 GPG 私钥永不存储,一键压缩也从不签名,保持无对话框、非交互。
