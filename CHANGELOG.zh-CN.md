...
- **「快捷指令」动作不再报「Couldn't communicate with a helper application」。** 运行 SimpleZip 的快捷指令动作时,系统会把应用当作 App Intents 助手在后台拉起,而它只有很短的窗口来应答连接。归档清单缓存此前被无上限地写进应用的主偏好存储,直到撑破系统对单个偏好值 4 MB 的硬上限——此后每一次启动都要反复重新序列化一份数 MB 的偏好文件而卡住,后台助手因此错过应答窗口。现在清单缓存改存它自己的磁盘文件(并带体积上限)而非偏好,遗留的超大条目会在启动最早期被清掉,可选的后台 AI 工作也改到应用真正进入前台后才启动——助手于是保持响应、动作正常执行。
