...
- **SimpleZip 记住你打开过的归档里有什么。** 打开归档时，它会把其中的非加密文件名缓存在本地——这是「某个文件在哪个压缩包里」查找的基础。设置 → 自动化 → 归档内容缓存让你掌控它：可以关掉（关掉即清空）、设置记住多少个归档、过多少天后忘记、查看当前占用、随时清空。隐私优先——加密归档永不记录（加密条目会被排除，整包加密的归档直接跳过），从别的归档里打开的（临时解压出来的）归档不记录，缓存不进设置备份，恢复出厂时清空。
...
- **设置「自动化」页。** 一页看全所有外部驱动方式：命令行工具（安装区从「通用」搬来——单一归属）、八个快捷指令动作与「打开快捷指令」按钮、Spotlight 索引开关（发布包与最近任务，默认开）、simplezip:// 命令（一律 app 内确认）、Finder 服务总览、按来源的活动中心使用统计，以及新安全开关「允许自动化使用预设密码」（默认开；关掉后无人值守的快捷指令/CLI 只试空密码）。该开关随设置备份迁移。
