...
- **开启静默后台索引会注册它自己的调度器,运行状态可检测并修复它。** 打开「静默后台索引」会注册一个后台服务,由系统按计划唤醒 —— 即便 SimpleZip 已关闭 —— 来保持 AI 索引新鲜;关闭则反注册。设置 → 运行状态会显示该服务是否已注册,并提供一键修复(重新注册以清除应用更新或更换签名身份后偶发的「陈旧」状态)。如果你在 系统设置 → 通用 → 登录项 把该服务关掉了,SimpleZip 会尊重你的选择:绝不偷偷重新打开,而是引导你去登录项自己重新启用。启动时,服务缺失会被静默补注册、应用更新后会被刷新;而你主动关掉的服务则保持不动。
