...
- **打开、浏览、解压「条目极多」的归档与文件夹不再卡死。** 打开大归档、浏览有几万个文件的文件夹、弹出解压对话框、生成解压任务,以及 Finder 自动解压 /右键「用 SimpleZip 解压」,过去都会卡住主窗口。现在这几条路径全程保持响应;无密码 ZIP 默认改用内置 7-Zip 引擎解压(macOS 引擎仍作兜底)。
