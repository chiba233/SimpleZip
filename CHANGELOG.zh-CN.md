...
- **端上 AI 正在迁出主程序、改到独立进程跑。** SimpleZip 的 AI 推理与后台索引正从主程序里抽出来:前台请求走内嵌的 XPC Service(按需拉起,不是登录项、也不受「允许后台」门控),后台索引走一个专门的 agent —— 即便 App 关着,launchd 也能按计划周期把它唤醒。共享的扫描、索引与内容预读代码现已下沉到 `SimpleZipCore`,由 App 与 agent 复用而非各拷一份 —— 同一份扫描编排、只走白名单的只读/预算受限元数据遍历、内容预读脱敏,以及「最久没扫优先」的渐进覆盖,在哪个进程里都是同一套。App 通过带版本的配置文件把 AI 设置同步给 agent,AI 主开关作为硬红线强制(主开关关 → agent 拒绝一切生成);agent 读白名单、跑一轮预算受限的扫描、把记录写回共享派生数据,并据配置间隔自节流,带单次运行时长上限(超预算的目录留待下次唤醒续上),并以 `ProcessType=Background` 交给系统做电源感知的延迟调度。为此声明了一个周期后台索引 LaunchAgent(默认关闭,仅在开启「静默后台索引」时激活);dev 与正式构建用各自隔离的 bundle id、Mach 服务名与 LaunchAgent 标签,确保自签 dev 版与已发布版永不冲突。每个端上模型 pass 现在都完全在那个引擎进程里、经统一的 `generate(kind:)` XPC 契约跑 —— 报告与解释的散文、文件行/归档行的建议、AI 文件夹的 plan / review / 分组 / 核查不扣题,以及自然语言的归档与设置查询 —— 主程序二进制因此不再创建任何语言模型 session,且每个 pass 的拼 prompt / 解析 / 校验都在共享引擎层,后台 agent 无需拉起主程序即可跑。连只读的「模型是否可用」查询现在也走引擎(经 XPC,结果缓存以便 UI 同步读取),主程序二进制因此**完全不再 import 端上模型框架**。
