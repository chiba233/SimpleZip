...
- **创建与解压对话框新增实时 AI 速览（macOS 26）。** 打开任一对话框，当 Apple Intelligence 可用时，预检概要下方会自动出现一段 AI 速览——无需点按钮——给出大致耗时感受和一条**基于真实顶层条目(而非只看计数)**的实用提醒——创建速览能看到顶层被打包的真实文件/文件夹(于是能提醒「把文件夹塞进 gzip 这种单文件格式会先打成 tar」或同名冲突等);解压速览能看到归档真实的顶层条目(于是能说出「解出来会是什么」),外加可能逃出目标文件夹的可疑路径、会被覆盖的文件、缺失分卷或磁盘空间不足。每条提醒都严格基于这些事实——绝不猜测它看不到的文件类型。它会随你改格式、级别或目标位置实时更新，全程本地推理，只描述/建议，绝不改设置、绝不跳过安全询问。一道很薄的分割线把它与确定性预检统计分开，并配一个刷新按钮可随时重算。所有 AI 生成都经一道串行闸**严格一个接一个**地跑、且绝不中途拆毁，因此快速切换格式/目标位置(或体积预估异步到达)都不会再让本机模型崩溃。AI 助手关闭或不可用时整体隐藏。
