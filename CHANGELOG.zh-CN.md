...
- **Finder 自动解压接入活动中心。** 每个 Finder / 右键解压都会在活动中心登记为任务（进度、取消、历史），解压浮窗的「取消」旁边多了「活动中心」按钮可一键跳过去。
- **新增：设置 → 通用 加了「Finder 扩展」一节。** 说明右键菜单项的用途，并直达 系统设置 → 扩展 去启用/停用扩展 —— 当 Finder 显示的是旧菜单时，在那里关掉再打开会强制重新登记当前这份构建。
