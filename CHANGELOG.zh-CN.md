...
- **「快捷指令」动作不再报「Couldn't communicate with a helper application」。** 运行 SimpleZip 的快捷指令动作时,系统此前会把整个应用当作 App Intents 助手在后台拉起,而完整启动流程太重、赶不上系统给的极短连接窗口,动作于是超时。现在归档类动作——解压、创建、测试、校验校验和、比较、搜索、检查、分析空间、体检、查重、可复现、审计、快验、救援——都改在一个专门的轻量 App Intents **扩展**进程里跑,由系统自己拉起、**完全不启动整个应用**,因此即刻应答。少数需要应用自身数据的动作(改设置、查哪个已打开归档含某文件、生成发布包)仍在应用内运行。加密归档由相关动作上一个可选的**密码**字段处理——留空时,只有当某个归档确实需要口令才会提示你输入。
- **打开 XIP 归档(如 Xcode 的 `.xip`)现在直接进入它的真实内容。** `.xip` 是带签名的容器,真正的载荷藏在内层 `Content` 成员里;此前 SimpleZip 要么一片空白(解析 bug 把 xar 条目的 `Type = file` 字段当成归档头标记、丢弃了每个条目),要么把原始的 `Content` / `Metadata` 外壳摆给你看却没有明显的入口。现在打开 `.xip` 会用系统 `xip` 工具展开它的载荷(顺带校验 Apple 签名),直接把你带进解压后的内容(以文件夹浏览)——完全等同打开 `.tar.zst` 看到的是内层文件、打开 `.siz` 看到的是其签名载荷:你绝不会看到那层 xar 外壳,地址栏仍显示原始 `.xip` 路径,「上一级」回到它所在的文件夹。展开过程显示常规的「正在读取压缩包…」状态,且可取消。在这个内容视图里**解压**动作现在也能用了——它直接把**已经展开好的内容**(你选中的项,或未选中时的全部)拷到你选的文件夹,**复用磁盘上已有的内容、不再重跑 `xip --expand`**;已挂载的 `.dmg` 同理从现有挂载点拷出——这也修掉了一个严重 bug:此前对已挂载的 dmg 解压可能把你正在浏览的卷卸载掉。列出或校验 `.xip` 也不再因每个 xip 的 `Content` 本就携带的良性尾部数据而误报「错误」。
- **「快捷指令」与 Siri 现在显示为你的语言,不再只有英文。** 每个快捷指令动作——名称、参数标签、一行摘要、Siri 触发短语——此前不论是 1.0.0 的归档动作还是更早的动作都只有英文。现在这些全部已在 10 种支持语言里完成本地化,于是动作列表、像「救援 [归档]」「快速核对 [文件夹] 中的发布组」这样的摘要、以及语音短语,都会显示为你的语言。

### 改进 (improvements)

**面向开发者**

- **体量大的派生/缓存数据迁出主偏好,改存独立文件。** 归档清单缓存、AI 工作区数据、活动历史此前各自作为一坨不断增长的数据塞在应用主偏好(UserDefaults)里,而单个偏好值有 4 MB 硬上限——一旦谁撑破,每次启动都要反复重新序列化一份数 MB 的偏好文件而卡住(这正是快捷指令「couldn't communicate」失败的根因)。三者现在都改存 Application Support 下各自的磁盘文件,各带一次性、防丢迁移,主偏好保持小、启动(含 App Intents 后台助手)保持快。存储后端抽象成一个小协议,顺带让测试套件能注入内存存储、不再往 ~/Library/Preferences 拉垃圾。
...
- **开启静默后台索引会注册它自己的调度器,运行状态可检测并修复它。** 打开「静默后台索引」会注册一个后台服务,由系统按计划唤醒 —— 即便 SimpleZip 已关闭 —— 来保持 AI 索引新鲜;关闭则反注册。设置 → 运行状态会显示该服务是否已注册,并提供一键修复(重新注册以清除应用更新或更换签名身份后偶发的「陈旧」状态)。如果你在 系统设置 → 通用 → 登录项 把该服务关掉了,SimpleZip 会尊重你的选择:绝不偷偷重新打开,而是引导你去登录项自己重新启用。启动时,服务缺失会被静默补注册、应用更新后会被刷新;而你主动关掉的服务则保持不动。另外,SimpleZip 自己开着时索引归它自己,后台服务会让位,不会同时重复做同一份活。
...
- **欢迎助手新增 AI 页,老用户升级后会弹一个简短的更新助手。** 首次设置现在包含一张端上智能页,你可以在这张卡里直接开关 AI 助手、建议,以及可选的后台索引 / 内容预读——进阶的后台开关放在开启助手后才弹簧冒出的第二张卡里。当你更新到新版本,不再是什么都没有(或重走整个欢迎),而是弹一个简短的更新助手,只展示这一版的新内容 + 一个「搞定」页。它是自动的:每张欢迎卡片都带一个「已看」标记,所以将来任何版本新增的卡片,都会自动出现在老用户的更新助手里,无需任何版本判断。
...
- **在地址栏粘贴网络归档 URL 即可「边下边解」。** 在地址栏输入指向 zip 或 tar 家族归档的 `http(s)` 链接(如 GitHub 的 `…/v1.0.zip`),会弹出专门的「下载并解压」面板,而不是当本地路径处理。它先探测服务器是否真给一个可流式的归档——显示 URL、解析出的文件名、以及明确的「可流式解压 / 不支持」状态,**绝不假装支持**——然后把下载流直接管进解压:字节边到边解压,**整个压缩包从不落盘**(可勾选额外保留一份下载的压缩包)。解压进你当前浏览文件夹下的子文件夹(可改),走和本地解压一样的 untrusted 安全校验;取消或失败都会清理干净。仅支持可流式的格式——加密归档、需要随机读取的格式按设计不支持。
...
- **发布历史可以删除了。** 发布助手的发布账本每条记录新增删除按钮,可清掉过期或误记的条目——只删记录、绝不动你的真实产物——发布包的 Spotlight 索引随之同步。
...
- **看似在后台的重活其实仍跑在主线程;取样定位到四处卡顿,现在都真正移出了主线程。** (1) 给 `async` 辅助函数标 `nonisolated` 在本工具链上**并不会**把它移出调用方 actor——`@MainActor` 调用方仍会在主线程跑完整个函数体,于是 `7zz` 列表解析卡在主线程(大归档实测 ~1.4s)。列表调用与 ZIP 中央目录加密探测现在用 `Task.detached` 包裹,这是唯一可靠 hop 到 cooperative pool 的手段。(2) 逐条目的 7-Zip / unzip 日期解析用 `DateFormatter`,首次解析会触发 ICU 加载 locale 符号表(冷启动数百毫秒)外加每次调用开销;现在按固定格式 `yyyy-MM-dd HH:mm:ss` 手动抽整数填进复用的 `Calendar`,彻底绕开 ICU。(3) 文件浏览器的 outline diff 用 `URL.standardizedFileURL` 建布局指纹,而它会对每个文件 `stat()`、且发生在**每次 SwiftUI reconcile**(实测 ~2.4s);现在改用纯路径字符串。(4) 菜单栏命令树会在**每次 FSEvents 触发(~120ms)**时重建,而其中一个启用状态 getter(`canDropIntoOpenArchive`)每次都 `fileManager.fileExists`——对打开的归档做一次 `stat()`,碰上慢速 / 断开的网络卷就挂死主线程(实测 ~2.5s);该 getter 不再碰文件系统(打开的归档已有 FSEvents watcher 盯着它是否消失)。ZIP 加密类型也改为打开归档时(off-main)检测一次、缓存进 session,解压 / 测试 / 打开条目 / 弹密码框都读缓存,不再各自重读整个中央目录(有的甚至读两遍)。这个加密探测现在也只读归档的中央目录(seek 到 EOCD 记录后只读那一段),而不是把整个文件 mmap 进来——在网络共享上(mmap 会退化成真实 I/O)探测一个几 GB 的 ZIP 不再把整包全拖过来。解压对话框复用打开时已加载的清单、解压前安全检查复用这批预载条目、解压后暂存扫描跑在后台,Finder /右键自动解压与一键创建浮窗的逐文件进度也和主窗口走同一个 ~80ms 的 `ProgressCoalescer` 合帧。
...
- **一眼看懂的安全评分（A / B / C）。** 路径安全报告现在最上面先给一个大白话风险等级——A（低）、B（中）、C（高）——不用读完整份发现清单也能知道这个包开起来安不安全。纯规则系统（绝不让 AI 判安全）：每条发现加上加密、macOS 垃圾、缺分卷、损坏都按严重度加权，**按发现的最严重那一项**定级——所以一堆鸡毛蒜皮的小提示绝不会被放大成「高风险」，而加密（本就不是风险）也绝不会拉低等级。评分一并写进导出的 Markdown / JSON 报告。
...
- **找出归档里的敏感与配置文件。** 归档菜单 → 查找敏感与配置文件:对打开的归档条目做一次快速的、按文件名的扫描,把值得注意的归类——看起来像私钥/凭据/机密的文件(`.pem`、`.key`、`id_rsa`、`.env`、名字含「secret」「token」等)、许可证与版权文件、配置(`.yaml`、`.json`、`.conf`、`Dockerfile` 等)和脚本。纯确定性、只按文件名匹配——从不检查内容,加密条目(列不出名字)也永不出现。macOS 26 上有个「解释」按钮,把扫描结果写成一段大白话,并温和提醒疑似机密类别勿随意外传——AI 只解释确定性结果,从不扫列表、不做判定。
...
- **报告可从活动中心重新打开、且跨重启保留。** 发布检查、.app 专项检查、发布目录检查、元数据报告、空间分析任务完成后，卡片上有「查看报告」按钮，不必为再看一眼报告而整个重跑；比较结果与发布检查/元数据报告随任务历史落盘，重启后照常可看（超大比较跳过持久化以免历史膨胀）。
...
- **用一句话切出任务的临时视图（macOS 26）。** 当 Apple Intelligence 可用时，活动中心头部有个 sparkles 按钮，输入一句大白话——「minecraft.zip」「今天失败的」「30 分钟内」「命令行跑的」——本机 AI 把它抽成精确条件（关键词、时间上限、状态、来源），再由 app 在**代码里精确匹配**:「zip」必中所有名字含 zip 的任务、「30 分钟内」必留所有 ≤30 分钟的任务,确定性的、每次都准。（模型只负责理解你的话,从不亲自扫列表选行,所以匹配可靠、绝不编造结果。）筛选生效时按钮填充紫高亮，点一下即清空；若这句话抽不出任何条件,按钮不点亮并给一行提示。每个任务卡也在状态同一排显示运行时间（如「2 小时前」「昨天」）。AI 助手关闭或不可用时整体隐藏。
...
- **用一句话找设置（macOS 26）。** 当 Apple Intelligence 可用时，设置侧栏顶部有个小 AI 搜索框：用大白话输入你想要的——「删除前别再问了」「让搜索更快」「把 GPG 那些关掉」——SimpleZip 就跳到对应设置（切到那一页、高亮那一行），任意语言都行。如果你明确说要开或关、且它是安全的便利开关，会顺手帮你切；安全敏感的设置绝不这样切（与语音控制同一套三道闸），只导航过去。模型只负责挑你指的是目录里哪一项——绝不改动 app 没明确放行的东西。AI 助手关闭或不可用时整体隐藏。
...
- **发布产物可以和上一次对比。** 发布历史抽屉里每条记录新增对比按钮：账面报告显示日期/体积/文件数变化、结构指纹是否变化（重新打包 vs 内容真变了）、「垃圾回潮」提醒，以及上次 vs 这次的发布卫生清单（可复现 / SHA256SUMS / 签名）——产物不在也能比。两个归档都还在时一键跑完整文件级对比（现有比较视图呈现）。macOS 26 上（Apple Intelligence 可用时）还有一个**白话总结**按钮，把这份账面对比变成一段大白话，专门点出值得复看的倒退——垃圾回潮、指纹意外变化、校验文件或签名比上次少了——只描述，绝不给「该不该发」的结论。
...
- **发布前质量门。** 六条规则——可疑条目路径、macOS 垃圾、空目录、未生成 SHA256SUMS、未签名为 .szs、App bundle 签名问题——每条可在新折叠抽屉里设为 关/警告/阻断（默认全关，不开启就什么都不变）。警告显示在检查报告顶部；阻断让这次运行失败并点名触发规则。没检查到的事实（检查关闭时）绝不当成问题。
...
- **发布目录完整性检查。** 右键文件夹 → 归档工具 → 检查发布目录：SHA256SUMS 覆盖与逐条实测、`.szs` 清单条目逐个对照目录文件、VERIFY.md 引用是否指向已改名/删除的文件、随包公钥能否独立验证随包 `.szs`（临时隔离钥匙环——绝不读写你自己的钥匙环），以及不属于任何发布角色的文件清单。全程只读；走统一报告导出菜单。GPG 关闭时公钥检查如实报「已跳过」。目录里的 `.dmg` 还会被只读挂载一下、确认里面确实有内容（空镜像或挂不上会被标出来）。
...
- **五个新的快捷指令动作。** 验证校验文件（逐条核验 SHA256SUMS 类文件，不安全路径直接拒绝，返回真/假）、比较归档（条目级比较，返回是否一致）、创建发布包（无头跑发布助手：打包 + 检查 + SHA256SUMS，可从列表里挑选工作区预设、或用变量按名字匹配；`.szs` 签名只在交互式流程里做，绝不无人值守）、搜索归档内容（返回路径包含你输入文本的条目——条目名需要密码的归档不会被列出）、检查归档（不解压就看文件数、总大小、macOS 垃圾与可疑条目路径，无标记时返回真）。归档名沿用统一校验；每次运行进活动中心并标记为快捷指令来源，发布运行照常进发布账本。
- **设置「自动化」页。** 一页看全所有外部驱动方式：命令行工具（安装区从「通用」搬来——单一归属）、八个快捷指令动作与「打开快捷指令」按钮、Spotlight 索引开关（发布包、最近任务与打开过的归档内容，默认开）、simplezip:// 命令（一律 app 内确认）、Finder 服务总览、按来源的活动中心使用统计，以及新安全开关「允许自动化使用预设密码」（默认开；关掉后无人值守的快捷指令/CLI 只试空密码）。该开关随设置备份迁移。macOS 26 上,快捷指令一节还多了个**建议一个自动化**按钮(Apple Intelligence 可用时):本机 AI 看你实际怎么用这个 app——最近的操作类型与触发来源计数(只看聚合数字,绝不看文件名或内容)——起草一两个贴合你习惯的 Shortcuts 点子 + 大白话设置步骤,落进可编辑文本框。这是给你自己照着搭的草稿,绝不替你建、装或运行任何东西。
- **任务记录来源。** 任务会记录发起表面——应用内、命令行、快捷指令/Siri、`simplezip://` URL 或 Finder 服务——并随任务历史持久化（来源筛选与自动化统计的地基；旧历史照常加载，老记录按应用内计）。
- **开发者工具：格式兼容性实验室。** 选一个小文件夹，实验室对真实后端把它在 zip、7z、tar、tar.gz 之间往返——实测什么真正留下来：结构、可执行位、扩展属性、符号链接、时间戳、注释往返（zip）与可复现性（zip/7z）。探针加在临时副本上，原文件夹绝不被改动。按设计藏在开发者工具里。AI 数据区也会显示动态文件夹的隐藏竞争池、已通过复核和复核中数量,所以「0 可见」时不再看不出生成器到底有没有在跑。
...
- **AI 起草你的发布文书（macOS 26）。** 三个端上 AI 生成器接进发布相关界面，每个都把结果落进可编辑文本框供你审阅后自取：**起草 VERIFY.md** 把发布目录检查结果变成可直接粘贴的校验说明——按目录里实际有的东西给出真正的 `shasum`/`gpg` 命令——外加一节「缺失 / 建议」；**起草发布正文** 把发布检查报告变成一段 GitHub Release 公告（下载、SHA-256 与校验步骤；只有当产物旁确实有签名 `.szs` 和公钥时才出现签名小节）；「复制为 Issue」旁的 **智能归类** 按钮则为诊断 issue 建议标题、标签和一段白话总结。全部只读、只吃报告里的具体事实（绝不编造文件或版本号）、端上推理，仅在 AI 助手可用时出现。
...
- **修改密钥 passphrase 不再把旧口令暴露给 `ps`。** 以前改密码流程把**旧** passphrase 当作 `gpg` 命令行参数传（`ps` / 活动监视器可见），只有新口令走 stdin。现在新旧口令都通过 `--pinentry-mode loopback --passwd` 走 stdin，符合 app「密钥只走 stdin」的惯例。（已对 GnuPG 2.5.x 实测验证。）
...
- **浏览 /Applications（及其他图标密集文件夹）不再卡顿。** 以前每个可见行每次重绘都在主线程取真实文件图标并解码 ICNS——对一套标准 /Applications 实测一轮约 740ms。现在图标从预栅格化位图缓存里取（绘制约 1ms）；首次见到的行先显示类型级占位图标，真图标在后台取回栅格化后原位换上。表格 cell 改为真正复用，不再每个滚动帧拆掉重建文本框/图片视图/约束；应用显示名也不再每次刷新文件夹重读 Info.plist。
...
- **热点路径加 os_signpost 打点。** 共享的 PerfSignpost 把后端子进程生命周期(每次 7zz/gpg/rar 运行)、列归档、安全/空间/发布检查扫描、比较、文件图标解码都括成 Points of Interest 区间,慢在哪用 Instruments 一眼可见。零行为变更、零用户可见,纯性能剖析管线。
