...
- **新增:搜索 / 过滤当前列表。** 窗口标题栏工具栏新增原生搜索控件——平时折叠成放大镜图标,点它或按 ⌘F(也在 编辑 → 查找 菜单)才展开成输入框——按你键入的文字过滤文件浏览或已打开压缩包的内容,名称或路径匹配的条目才显示。清空搜索框即恢复完整列表。(用 AppKit `NSSearchToolbarItem` 实现真正的 macOS 折叠效果:SwiftUI 的 `.searchable` 在 macOS 上渲染成不折叠的大框,而折叠修饰符仅 iOS 可用。)
