...
- **五个新的快捷指令动作。** 验证校验文件（逐条核验 SHA256SUMS 类文件，不安全路径直接拒绝，返回真/假）、比较归档（条目级比较，返回是否一致）、创建发布包（无头跑发布助手：打包 + 检查 + SHA256SUMS，可按命名工作区预设；`.szs` 签名只在交互式流程里做，绝不无人值守）、搜索归档内容（返回路径包含你输入文本的条目——条目名需要密码的归档不会被列出）、检查归档（不解压就看文件数、总大小、macOS 垃圾与可疑条目路径，无标记时返回真）。归档名沿用统一校验；每次运行进活动中心并标记为快捷指令来源，发布运行照常进发布账本。
