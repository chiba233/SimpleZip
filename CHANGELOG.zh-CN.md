...
- **主窗口支持系统原生标签页（⌘T）。** 按 ⌘T 开新标签；SimpleZip 已在运行时从 Finder 打开压缩包或文件夹，会在新标签里打开，而不是替换掉你正在看的内容。这是真正的 macOS 窗口标签 —— 只有一个标签时标签栏自动隐藏，「窗口」菜单里的「显示标签栏」/「移动标签到新窗口」/「合并所有窗口」都能用。每个标签完全独立（各自的文件夹、选区、撤销历史）。设置、关于、更新窗口仍是独立窗口，绝不并进标签组。
