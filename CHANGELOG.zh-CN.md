...
- **发布目录完整性检查。** 右键文件夹 → 归档工具 → 检查发布目录：SHA256SUMS 覆盖与逐条实测、`.szs` 清单条目逐个对照目录文件、VERIFY.md 引用是否指向已改名/删除的文件、随包公钥能否独立验证随包 `.szs`（临时隔离钥匙环——绝不读写你自己的钥匙环），以及不属于任何发布角色的文件清单。全程只读；走统一报告导出菜单。GPG 关闭时公钥检查如实报「已跳过」。
