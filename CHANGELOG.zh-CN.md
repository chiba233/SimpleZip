...
- **全新的 AI 侧栏:动态主题文件夹。** 主窗口侧栏新增 AI 区,是一组只读的「主题文件夹」——把按语义属于同一主题的东西聚到一起(共享名字 token、处理过它们的同一个任务、或内容相关的 文件 / 归档内条目 / 任务 / 报告),无论它们物理上散在哪,呈现成一个连贯的虚拟文件夹,带子分组和建议动作(在 Finder 显示、打开、哈希、测试;任何写盘动作一律回原生确认流,绝不自行删除、移动或解压)。它们在后台被发现——哪怕你从没在 app 里打开过那个文件夹——来源是一个 **opt-in、仅白名单、只读** 的文件索引(你授权扫哪些目录;只取元数据——名称、大小、日期——绝不读内容;系统 / 密钥 / 缓存 / 临时目录永远排除)外加你的活动历史。只有完整、够分量的主题才会被推荐、且数量有上限;列表按「主题强度 + 你打开它的频率 + 最近度」软加权排序,点一下绝不会把弱文件夹钉到最上面;每个推荐文件夹都有一个「x」——不感兴趣——会对该主题永久降权、并随时间衰减。整段受 AI 助手开关门控,后台索引在你开启并授权一个目录前保持关闭。
