...
- **新功能：启动位置扩展**
  ...
  - 新增「自定义位置」：选了之后立即弹出文件夹选择面板，选完后启动位置那行会显示带文件夹图标的目录名（悬停看完整路径），旁边有「选择…」按钮可重新挑。取消第一次选择时 Picker 自动回退到之前的选项，避免「Custom 但没路径」的卡壳。
  - 若选过的文件夹后续被外部删 / 移走了，描述行会显示红色提示，启动时静默回落到个人文件夹（不会因为路径失效启动失败）。
- **Bug 修复**
