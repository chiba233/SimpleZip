...
- **新增:比较两个归档(Archive Diff)。** 在文件浏览里选中两个归档,右键 →「比较归档」(或只选一个、通过「与其他归档比较…」挑第二个)。SimpleZip 会列出两个包并弹出比较结果:只在其中一个包里的条目、两边都有但不同的条目——逐条标明到底差在哪(大小、CRC、修改时间、文件 ↔ 文件夹、变为加密 / 未加密)——以及相同条目计数。整份报告可一键复制为纯文本,比较过程作为可取消任务进活动中心。措辞使用中性的「仅在 A 中 / 仅在 B 中」——两个任意的包没有天然的「旧 / 新」之分。
