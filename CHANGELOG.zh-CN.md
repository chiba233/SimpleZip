...
- **新增:压缩预设,以及供 Finder「压缩」一键服务使用的默认预设。** 设置 → 压缩 → 压缩预设 里可以把常用压缩设置(格式、等级、7-Zip 方法 / 固实…)存成命名预设,并在一个随所选格式自适应的专用编辑器里增删改。把其中一个设为**默认**,Finder 的「压缩为 ZIP / 7z / TAR.GZ」一键服务(及对应 NSService)就会自动套用它的可复用设置——每个入口保留自己的输出格式,只是用预设的等级 / 方法。密码和 GPG 签名私钥永不存进预设,一键压缩也从不签名,保持无对话框、非交互。
