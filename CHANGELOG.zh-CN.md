...
- **修复:Finder 右键「用 SimpleZip 创建 ZIP / 7z / TAR.GZ」是个黑盒,还会拽出主窗口。** 这条一键创建之前裸跑后台任务 —— 不进活动中心、没有进度、没有日志,失败时反而把主窗口拉到前台。现在和主窗口里的创建走同一套受管任务:在**活动中心**里显示进度、命令日志、成功 / 失败,可取消;全程不碰主窗口,完成后在 Finder 里高亮产物。（解压早就是这套,创建这条之前漏了。）
