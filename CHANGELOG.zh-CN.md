...
- **右键任意文件 / 文件夹 →「加密为 .gpg…」。** 一个紧凑的 sheet 让你选收件人公钥、对称密码或两者皆选（任一收件人的私钥**或**该密码都能解密——SimpleZip 从不经手你的 GPG 私钥 passphrase；若你设了预设密码，对称密码框会自动填入）。单个文件变成 `name.gpg`；文件夹或多选会先打包成一个 `.tar`（中转明文落进加密临时卷，绝不裸写普通磁盘）再加密成 `name.tar.gpg`。产物落在源文件旁、在 Finder 中选中显示，整个过程作为可取消的活动中心任务运行。仅在「设置 ▸ GPG」启用 GPG 集成时显示。
...
- **打开 `.asc` 密钥文件即可导入到钥匙串。** `.asc` 现已注册为文件类型（Alternate 优先级，不会抢占你默认的文本编辑器关联）：打开它——无论在 SimpleZip 浏览器里双击还是从 Finder 打开——都会先嗅探内容，若是公钥/私钥导出则弹出「导入到用户钥匙串或 SimpleZip 私有钥匙串」对话框。同样的「先看内容再路由」逻辑在 app 内浏览器对 `.gpg`/`.pgp`/`.asc` 一并生效，且仅在「设置 ▸ GPG」启用 GPG 集成时启用（否则用系统默认 app 打开）。
