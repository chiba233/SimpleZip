...
- **移除代码库中未接线的 AI 脚手架。** 早期一套远更宏大的 AI 设计里建好、但从未接进 app 的若干 Core 脚手架被删:多引擎能力协商层(已被 app 实际使用的更简单的可用性门取代)、统一 AI 意图路由、AI 命令排练 / 批处理规划器、虚拟文件夹动态主题刷新——连同它们的单测一并删除。它们零引用,无任何用户可见变化。
