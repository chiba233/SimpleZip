...
- **统一的端上 AI 数据层(更强但依旧隐私的 AI 的底座)。** SimpleZip 正在搭建一套共享、完整单测的底座来决定「怎么把数据喂给苹果端上模型」——定位是「本地事实索引 + 小模型智能组织层」:App 负责把本机事实采集、过滤、脱敏、按预算整理成结构化信封,每个信封都带明确的隐私描述块(端上执行、标准/深度本地上下文强度、确认排除的红线类别)和「省略说明」(哪些数据没给、为什么),外加可复用的证据卡和经校验的来源引用;模型只负责解释、命名、排序、归组、起草。模型返回的每个路径、任务 id、归档条目 id 都会和 App 实际提供的候选校验,任何危险动作一律回到原生确认流。口令、密钥材料、加密归档条目名、密文、解密明文绝不进入这一层——靠契约保证(确定性编码,发给模型的事实可单测、可导出审查)。信封之上还有一层确定性的「组织」基元,模型只做精修:归档画像与角色识别、失败诊断标签与按标签的修复手册、低敏位置上下文、按任务/按归档的 AI 索引、文件类型分类与文件夹画像(角色 + 推荐视角派生)、可保存的查询计划、把已索引对象重新归组的「视角」(发布/源码/失败/签名/清理)、一眼看清归档内部构成的「内部地图」、确定性的下一步动作排序器、把模糊搜索重写成可执行本地查询、虚拟 AI 工作区、带硬性默认排除规则(系统/密钥/缓存/开发依赖/临时解密目录)的白名单后台预读作用域,隐式的兴趣/反馈信号、按时段感知的启动目录排序器,全局建议总线契约(每个界面位置发请求→拿回经校验、可忽略的建议卡),以及确定性的「为什么没有推荐」解释器——每一个都是纯值类型,无模型也能工作,且全部可单测。
