...
- **最近的发布包出现在 Spotlight 里。** 发布助手记录的每个发布都会按版本、日期、产物文件名、SHA-256 前缀索引进 Spotlight（macOS 15+）——搜版本号就能直接找到打好的包。只索引这些可公开的元数据；完整路径、结构指纹、归档内容绝不索引。每次启动（及每次新发布后）重新同步；macOS 14 及更早直接跳过。
...
- **设置「自动化」页。** 一页看全所有外部驱动方式：命令行工具（安装区从「通用」搬来——单一归属）、六个快捷指令动作与「打开快捷指令」按钮、simplezip:// 命令（一律 app 内确认）、Finder 服务总览、按来源的活动中心使用统计，以及新安全开关「允许自动化使用预设密码」（默认开；关掉后无人值守的快捷指令/CLI 只试空密码）。该开关随设置备份迁移。
