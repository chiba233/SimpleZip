...
- **Group By（文件浏览器）—— 把文件分成可折叠的区块。** 新增「视图 →文件按…分类」菜单，可选「不分类」（默认）或「按种类」。按种类时文件被切成可折叠区块（图片、文档…），区块默认展开、可手动折叠。和「隐藏文件」组复用同一套 NSOutlineView 基建。文件浏览器和压缩包浏览器都支持，各有独立的「视图」菜单入口（「文件按…分类」/「压缩包按…分类」）。
