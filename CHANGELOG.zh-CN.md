...
- **开发者工具:端上 AI 引擎 + 后台调度监视器。** 隐藏的 AI 数据实验室新增一组只读探针,观测迁出主进程后的管线:统一 `generate(kind:)` 契约自检、被动引擎 pass 计数(引擎自启动以来跑过哪些 pass、多少次、成不成)、后台调度预览(据当前运行时与信号 `AIBackgroundPlanner` 会规划哪些 tier-gated job)、工作区证据缺口读数(哪些工作区成员还缺校验和 / 归档完整性测试 —— 正是驱动调度器后台补哈希 / 补测的输入)、pending 检查队列读数(下游 —— 待插电执行的检查与已完成数)。全部确定性、瞬回、不碰模型,结果常驻可复制状态卡。
