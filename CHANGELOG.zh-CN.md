...
- **活动中心：计算哈希的结果现在会留下来。** 哈希任务以前算完只剩一个「完成」、毫无内容;现在展开详情会用和「文件哈希」弹窗**一样的格式化卡片**渲染(每个文件:大小、路径、CRC32/MD5/SHA-1/SHA-256/SHA-512),带「全部复制」——关掉弹窗后哈希也还在。哈希仍归「文件操作」。
- **活动中心：粘贴 / 移动会记录它做过的哈希比对。** 覆盖前比对内容时,任务详情会列出每个文件的源文件哈希 vs 目标文件哈希,以及结果(内容相同→已跳过 / 内容不同→已覆盖)。
