...
- **「快捷指令」与 Siri 现在显示为你的语言,不再只有英文。** 每个快捷指令动作——名称、参数标签、一行摘要、Siri 触发短语——此前不论是 1.0.0 新增的归档动作还是更早的动作都只有英文。现在这些全部已在 10 种支持语言里完成本地化,于是动作列表、像「救援 [归档]」「快速核对 [文件夹] 中的发布组」这样的摘要、以及语音短语,都会显示为你的语言。
