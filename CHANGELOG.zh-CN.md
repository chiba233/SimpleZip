...
- **散落的 App 现在会建议移到应用程序。** 当 `.app` 出现在下载文件夹、解压结果或已挂载的镜像里时,该行可展开出建议抽屉,一键「将 <名称> <版本> 移到应用程序」——与普通转移相同的冲突处理和活动中心任务,源卷只读时改为复制。**打开的压缩包里**的 `.app`(zip、7z、tar 系——凡能列出条目的格式)同样出这条建议:展开该条目行,一键把它解出并移入应用程序,沿用归档的安全检查与口令流程。识别只读取 App 包自身的版本信息——压缩包内的在展开该行时实时读取、读完即弃、绝不缓存;全程无 AI,助手关闭时同样可用。
- **「快捷指令」与自动化改走 `simplezip://` URL scheme。** 在快捷指令里用「打开 URL」动作(或任意脚本)即可按路径解压、创建、哈希、打开、校验或比较归档。你自己的快捷指令用每台机一份的可信密钥免去确认提示;来自其它 app 的 `simplezip://` 会先弹确认(可开关)。此前的 App Intents 归档动作已移除——它们在非沙箱应用上无法可靠回传结果——剩七个导航类动作(打开归档 / 文件 / 发布包 / 任务 / 设置、查找包含某文件的归档、更改设置)。

### 交互与界面 (UX)

**面向用户**

- **压缩包里的 App 现在有 App 的样子。** 打开的压缩包中,`.app`(以及安装器包)条目显示应用程序图标而非普通文件夹,右键菜单新增与访达一致的「显示包内容」用于浏览包内。双击仍是解出并打开,前置的活动内容安全确认保持不变。
- **Finder 任务小窗现在像普通窗口一样听话。** Finder 自动解压和右键「压缩为…」的进度小窗不再始终置顶，也可以最小化；运行时的当前文件行会固定预留一行，即使后端暂时没报文件名，下面的按钮也不会来回跳。

### 修复 (bugfix)

**面向用户**

- **打开 XIP 归档(如 Xcode 的 `.xip`)现在直接进入它的真实内容。** `.xip` 是带签名的容器,真正的载荷藏在内层 `Content` 成员里;此前 SimpleZip 要么一片空白(解析 bug 把 xar 条目的 `Type = file` 字段当成归档头标记、丢弃了每个条目),要么把原始的 `Content` / `Metadata` 外壳摆给你看却没有明显的入口。现在打开 `.xip` 会用系统 `xip` 工具展开它的载荷(顺带校验 Apple 签名),直接把你带进解压后的内容(以文件夹浏览)——完全等同打开 `.tar.zst` 看到的是内层文件、打开 `.siz` 看到的是其签名载荷:不会显示那层 xar 外壳,地址栏仍显示原始 `.xip` 路径,「上一级」回到它所在的文件夹。展开过程显示常规的「正在读取压缩包…」状态,且可取消。在这个内容视图里**解压**动作现在也能用了——它直接把**已经展开好的内容**(你选中的项,或未选中时的全部)拷到你选的文件夹,**复用磁盘上已有的内容、不再重跑 `xip --expand`**;已挂载的 `.dmg` 同理从现有挂载点拷出——这也修掉了一个严重 bug:此前对已挂载的 dmg 解压可能把你正在浏览的卷卸载掉。列出或校验 `.xip` 也不再因每个 xip 的 `Content` 本就携带的良性尾部数据而误报「错误」。
- **「快捷指令」与 Siri 现在显示为你的语言,不再只有英文。** 现存的每个快捷指令动作——名称、参数标签、一行摘要、Siri 触发短语——都已在 10 种支持语言里完成本地化,动作列表和语音短语都显示为你的语言。
- **删除文件后键盘焦点会停在邻近文件上。** 把文件移到废纸篓后,选区会落到列表里紧挨它的那个文件——这样就能用键盘连续删除,而不是焦点消失、每次都要用方向键重新选回一行。邻居跟着你眼睛看到的来:折叠的「隐藏文件」分组会被跳过,但若该分组是展开的,就会落到紧邻的隐藏文件,与 Finder 一致。

### 改进 (improvements)

**面向用户**

- **启动更快,后台维护更轻。** 活动历史不再在启动时解码——改为后台加载再合并进来,任务历史再长也不拖慢首个窗口(和快捷指令的就绪);AI 助手的索引由「每条建议写一次」改为「每批建议写一次」;从大归档里解压多个文件夹时,不再按每个所选文件夹重扫一遍条目清单;浏览过程中,文件列表只在建议真的变化时才重算建议指纹,而不是每个刷新周期都算一遍。

**面向开发者**

- **体量大的派生/缓存数据迁出主偏好,改存独立文件。** 归档清单缓存、AI 工作区数据、活动历史此前各自作为一坨不断增长的数据塞在应用主偏好(UserDefaults)里,而单个偏好值有 4 MB 硬上限——一旦谁撑破,每次启动都要反复重新序列化一份数 MB 的偏好文件而卡住。三者现在都改存 Application Support 下各自的磁盘文件,各带一次性、防丢迁移,主偏好保持小、启动保持快。存储后端抽象成一个小协议,顺带让测试套件能注入内存存储、不再往 ~/Library/Preferences 拉垃圾。
- **移除代码库中未接线的 AI 脚手架。** 早期一套远更宏大的 AI 设计里建好、但从未接进 app 的若干 Core 脚手架被删:多引擎能力协商层(已被 app 实际使用的更简单的可用性门取代)、统一 AI 意图路由、AI 命令排练 / 批处理规划器、虚拟文件夹动态主题刷新、创建/解压「建议卡」层(已被 app 实际使用的「一句话速览 + 套用推荐设置」取代)、以及几个自然语言能力(归档对比解释、搜索重写、自由文本选择——小型端上模型做不可靠)——连同它们的单测一并删除。它们零引用,无任何用户可见变化。
...
- **AI 建议落在文件浏览器里,而不是另起一个侧栏。** 1.0.0 的 AI 工作从可见 AI 侧栏收回到现有文件/归档行里:归档行可展开模型生成的一句话定性和动作建议;文件行可显示模型挑出的 SHA-256、推荐打开方式、网页链接、相关活动、安装提示、包内相关文件和只读内联报告;后台缓存的文件组会渲染成额外的可折叠 AI 行,但真实文件行照常保留。写死的抽屉建议已经移除——模型没有产出就保持空抽屉——任何会写盘的动作仍回到 SimpleZip 原生确认/任务流程,只读检查可以在行内显示结果。
- **后台 AI 索引和预读成为这些建议的 opt-in 基础设施。** 后台 worker 保留仅白名单、只读的索引与内容预读开关,用 AI 排序文件和归档预读队列,按指纹跳过未变化项,遵守电源/活跃度预算,并可把哈希/测试这类安全只读检查排入队列,在条件允许时稍后执行。
- **SimpleZip 关闭时也会在后台预烘焙 AI 建议了。** 开启「静默后台索引」后,后台助手不再只收集元数据 —— 它会自己跑端上模型,用你的界面语言把真正的建议预先烘焙好,这样你下次打开应用时建议已经在那了,而不是现算。现在覆盖全套:文件一句话摘要与逐文件动作、网页链接建议、磁盘镜像「拖到应用程序」提示、包内「你可能要的文件」与一句「这是什么包」、按文件/类型的工具栏动作排序、文件夹成组与「整理进新文件夹」建议、文件「有活动」提醒(命中近期任务产物的文件给一句「最近对它做过什么」)、排队的只读检查(哈希、完整性测试、以及仅当有异常时的一句路径安全 / 发布包检测说明),还有活动中心工作台(失败解释、命名的真实聚集、「现在最该处理什么」、筛选排序)。它会一直烘焙到单轮时间预算用尽(整批积压、不是前台那种每轮就几个的上限),下次计划运行时接着上次的继续;首段恰好切在多字节字符中间的大体积中日韩文档现在也能正确生成摘要了。它读的是与应用同一套 7-Zip/RAR 后端和设置,所以能自己列目录、测试归档。
- **开启静默后台索引会注册它自己的调度器,运行状态可检测并修复它。** 打开「静默后台索引」会注册一个后台服务,由系统按计划唤醒 —— 即便 SimpleZip 已关闭 —— 来保持 AI 索引新鲜;关闭则反注册。设置 → 运行状态会显示该服务是否已注册,并提供一键修复(重新注册以清除应用更新或更换签名身份后偶发的「陈旧」状态)。如果你在 系统设置 → 通用 → 登录项 把该服务关掉了,SimpleZip 会尊重你的选择:不会自动重新打开,而是引导你去登录项自己重新启用。启动时,服务缺失会被静默补注册、应用更新后会被刷新;而你主动关掉的服务则保持不动。另外,SimpleZip 自己开着时索引归它自己,后台服务会让位,不会同时重复做同一份活。
- **工具栏的上下文按钮开始学习你的习惯——开 AI 后还会按文件预烘焙最有用的动作。** 工具栏上那两个随选择变化的按钮,现在从与右键菜单同一份完整动作集里取候选(不再是写死的小列表),并按你实际常用的动作重排——无论你是从工具栏还是右键菜单点的——让每种选择下最常用的浮到前面。复选共用同一组建议,某格式不支持的动作根本不会出现,所以没有点不了的灰按钮。设置 → 通用 新增「学习我的工具栏习惯」开关(默认开,全本地,绝不记录文件内容)。开启 AI 助手后,后台还会为它已认为值得给建议的文件(其余按文件类型)预先烘焙「哪些工具栏动作对这个文件最有用」,叠加在你的习惯之上,让最相关的动作排在最前;关 AI 则纯按习惯排序。
- **欢迎助手新增 AI 页,老用户升级后会弹一个简短的更新助手。** 首次设置现在包含一张端上智能页,你可以在这张卡里直接开关 AI 助手、建议,以及可选的后台索引 / 内容预读——进阶的后台开关放在开启助手后才显示的第二张卡里。当你更新到新版本,不再是什么都没有(或重走整个欢迎),而是弹一个简短的更新助手,只展示这一版的新内容 + 一个「完成」页。它是自动的:每张欢迎卡片都带一个「已看」标记,所以将来任何版本新增的卡片,都会自动出现在老用户的更新助手里,无需任何版本判断。
- **可选的「流式快速解压」(zip 与 tar 归档)——整包**与**选定条目**都支持。 整包解压与「解压选中条目」两个对话框都为无密码的 zip 与 tar 家族归档(.zip、.tar、.tar.gz / .tgz、.tar.bz2 / .tbz2、.tar.xz / .txz、.tar.zst)新增「流式快速解压」开关:按归档里的物理顺序读取,而不是靠目录索引到处跳——在网络共享或超大包上快得多;选条目解压时则在同一趟顺序扫描里按名字匹配你选的那些条目。它需手动开启,且仅在无密码时才出现;打开后会隐藏它用不到的密码 / 解密方式选项,万一流式读取失败会自动回退到标准引擎。
- **在地址栏粘贴网络归档 URL 即可「边下边解」。** 在地址栏输入指向 zip 或 tar 家族归档的 `http(s)` 链接(如 GitHub 的 `…/v1.0.zip`),会弹出专门的「下载并解压」面板,而不是当本地路径处理。它先探测服务器是否真给一个可流式的归档——显示 URL、解析出的文件名、以及明确的「可流式解压 / 不支持」状态,**不会虚假显示支持**——然后把下载流直接管进解压:字节边到边解压,**整个压缩包从不落盘**(可勾选额外保留一份下载的压缩包)。解压进你当前浏览文件夹下的子文件夹(可改),走和本地解压一样的 untrusted 安全校验;取消或失败都会清理干净。仅支持可流式的格式——加密归档、需要随机读取的格式按设计不支持。

### 交互与界面 (UX)

**面向用户**

- **活动中心默认开得更大。** 默认尺寸调大了,不再那么挤。
- **设置页统一 hero 外观、并保持沉浸。** 每个设置分区顶部现在都有彩色 hero 头(图标瓦片 + 标题 + 一句内容描述),侧边栏用与活动中心同款的渐变瓦片。窗口默认开得更高、宽高都能自由调整(也能全屏,和活动中心一样),并全程保持毛玻璃、铺到边的沉浸式效果。
- **AI 设置有了自己的页面。** AI 助手开关从「自动化」页搬到了独立的 **「AI 与智能建议」** 设置页——给它一个自己的家,并附一句隐私说明:助手完全在你的 Mac 本机运行,绝不会收到密码、加密内容或密钥。页面里的「清空」按钮现在如实标明它的作用——清掉 AI 在后台积累的全部数据(文件索引、建议和学到的排序),绝不动你的真实文件、也绝不动 Spotlight 结果。
- **AI 设置页把隐私边界展开说明。** AI 页新增可折叠的「AI 隐私须知」,说明哪些本机数据会被索引、哪些永远不读,以及端上模型怎样使用这些数据。
- **活动中心新增只读 AI 工作台。** AI 助手开启时,任务列表页右侧会出现一个工作台,读当前页面、告诉你现在最值得先处理什么、为什么。列表沿用熟悉的时间序、在跑的任务置顶;工作台总结当前列表、提示未读失败。展开一个失败任务会给出一句话的失败原因解释——文件移动 / 复制这类没有后端日志的操作,现在会把逐文件的失败原因喂进这段解释,让它具体而非泛泛——旁边有「打开完整 AI 解释」按钮,以及一键的下一步动作,都接到这个任务自己的安全处理器上:打开报告、从失败步继续、重跑、改参重跑、复制诊断。它的建议筛选现在是 AI 命名的真实聚集:App 确定性发现真正反复出现的任务组(跨来源 / 类型 / 诊断维度交叉)——既有失败组、也有常用操作组,没失败时工作台也有建议——端上模型给值得展示的起一个自然语言短名(如「从 Finder 解压失败的」「所有压缩任务」)——模型没产出时,写死的分类筛选保留作兜底;你常点的筛选会随时间浮到最前。旁边还有 AI 推荐的**时间维度**——今天 / 本周 / 本月——并标出哪个窗口值得关注(最近一个有未读失败的);时间维度和建议筛选可以**双重叠加**,于是你能同时收窄到比如「今天」**且**「从 Finder 解压失败的」。每个生效的筛选和时间维度都列在统一的「生效中的筛选」栏里、各自一键清除,工作台左缘可拖拽调宽。「学习到的习惯」摘要显示你近期常用的来源、格式和位置(只摘要,绝不含完整路径);当某个手动操作反复出现,「自动化建议」会提示把它做成快捷指令或命令行工作流(只提示并打开快捷指令,不会自动创建)。一切都从本机任务索引派生、后台预烘焙、只读分析:它只排序、解释、筛选或定位任务,绝不重新运行或改动任何东西。
- **帮助页覆盖最新功能。** 设置 → 帮助 新增「AI 辅助」分区——AI 建议、智能推荐设置与工具栏、后台预读与隐私——并补充「流式快速解压」「从网址边下边解」两张卡片。
- **关于页文案焕新。** 简介改为突出当下亮点:快、私密、一切都在你的 Mac 上完成,支持对超大或网络上的压缩包边下边解,以及可选的端上 AI 辅助。
- **欢迎助手与更新助手把「取消」改为「跳过」。** 左侧按钮现在叫「跳过」,清楚表明它只是略过助手、而非撤销你已经做出的选择。

**面向开发者**

- **发布历史可以删除了。** 发布助手的发布账本每条记录新增删除按钮,可清掉过期或误记的条目——只删记录、不影响真实产物——发布包的 Spotlight 索引随之同步。
- **`simplezip` 命令行与「快捷指令」补齐缺失的归档动词。** 命令行新增 `list`(条目按 类型/大小/名称,或 `--json`)、`inspect`(发布助手的发布包检测、不解压——文件/文件夹数、总大小、macOS 垃圾、空目录、可执行、符号链接与可疑条目路径;有可疑则 exit 1)、`extract`(解进唯一命名文件夹,走与 Finder 自动解压同一条受检安全路径,`--to` 指定目标)和 `hash`(CRC32/MD5/SHA-1/SHA-256/SHA-512,默认 SHA-256,BSD-tag 输出可被 `verify` 读回);还补齐了原先只在 GUI 的分析与发布工具:`space`(空间分析——最大文件/目录/扩展名、压缩率、垃圾占比)、`rescue`(损坏归档尽力救援到 `(rescued)` 文件夹)、`checkup`(对多个归档或整个文件夹批量做完整性测试 + 可疑/垃圾/加密计数)、`duplicates`(按结构指纹查重复归档)、`reproduce`(同一文件夹打包两次,报告是否逐字节一致及哪些因素未归一)、`audit`(检查发布目录的 SHA256SUMS 覆盖、陈旧条目、失效 VERIFY 引用与孤儿文件)和 `verify-group`(只按文件名快速判断「这个发布组可被校验吗」)。全部支持 `--json`/`--quiet`、记入活动中心,加密归档会弹口令小窗(或读 `SIMPLEZIP_PASSWORD`)——绝不走命令行;shell 补全(zsh/bash/fish)同步覆盖。「快捷指令」也补上对应动作——**计算文件哈希**(返回十六进制摘要以串联),外加 **空间分析**、**数据救援**、**批量体检**、**查找重复归档**、**可复现检查**、**检查发布目录**、**快速核对发布组**——让同一批工具在快捷指令里也能自动化,不止命令行。

### 修复 (bugfix)

**面向用户**

- **没有苹果开发者签名时,「快捷指令与 Siri」会被明确标为不可用。** macOS 26 的 App Intents 执行链要求 validated bundle（带 Team Identifier 的代码签名），所以 ad-hoc / 未签名构建永远无法跑 SimpleZip 动作——系统会拒绝连接、快捷指令报「无法与 App 通信」。这些动作仍会注册进系统、出现在「快捷指令」App 里（app 无法把它们从那儿移除），所以与其在设置里整体隐藏,设置 → 自动化 现在改为照常显示该节、并打上醒目的「不可用」说明,运行状态页也会给「快捷指令与 Siri」标黄并解释原因。构建被正确签名后一切自动恢复。CLI、URL scheme、Finder 服务、Spotlight 等通道不受影响。
- **关掉 AI 助手时,它的按钮现在会立刻消失。** 以前各处 AI 入口(创建/解压对话框里的内联速览、报告与失败任务上的「解释一下」按钮、AI 搜索框)只在视图下次重建时才重新检查主开关,所以在设置里关掉后它们不会立刻消失(再打开也不会立刻出现)。现在主开关一变,它们即时显隐。
- **AI/Spotlight 索引不再容易拖慢启动。** Spotlight 持久化索引会跳过未变化项、串行运行、遵守电源档,并把加密卷相关工作移出主线程。
- **AI 后台任务不再跑完一轮就停住。** 后台索引心跳会继续运行,并在 scope 之间渐进轮转,所以缓存建议和诊断数据能持续更新。
- **归档行 AI 动作重新能冒出来。** 模型挑出的发布检测、测试、哈希等归档建议不再卡在充电门后;基于活动记录的建议也改用任务完成后的真实产物路径。
- **运行状态的「上次检查」时间现在会持续走字。** 它过去可能卡在「刚刚」不动——相对时间只在面板偶发重绘时才重算,刚检查完就可能停在 0;现在每秒自动更新。
- **文件夹级的发布工具不再要求你先选中一个子文件夹。** 「快速核对发布组」「检查发布目录」「可复现性检查」和文件夹体检,过去在什么都没选时会报「请先打开或选择一个支持的压缩包」——哪怕你已经身处想检查的那个文件夹里。现在没选东西时,它们就作用于你当前正在浏览的文件夹;「查找疑似重复的归档」也会给出准确的「本文件夹里至少需要两个归档」,而不再是那句误导的归档提示。
- **打开、浏览、解压「条目极多」的归档与文件夹不再卡死。** 打开大归档、浏览有几万个文件的文件夹、弹出解压对话框、生成解压任务,以及 Finder 自动解压 /右键「用 SimpleZip 解压」,过去都会卡住主窗口。现在这几条路径全程保持响应;无密码 ZIP 默认改用内置 7-Zip 引擎解压(macOS 引擎仍作兜底)。打开超大归档现在也省内存得多,几百万条目的大包不再有被系统因内存超限杀掉的风险。
- **展开 AI 建议后点一下不再把它收起。** 在嵌套文件夹里,打开某文件的 AI 抽屉再点击任意处,可能把刚展开的抽屉收起。现在新露出的行不会被误判为「内容变了」,你刚打开的抽屉保持展开。
- **一轮系统性安全加固。** 收紧了多处安全面:不可信的归档条目名 / 文件名不再被内置与系统工具当成命令开关;输出与解压目标不再能被引导到你所选文件夹之外(含下载文件名与手填的归档名);签名先写临时文件、成功后才原子替换原文件,取消或失败的签名不再丢失你已有的文件;DMG 解压不再复制指向镜像外的符号链接;导入损坏的设置备份不再崩溃;归档条目名保留精确的前后空格,删除与重命名作用在正确的条目上;可选 AI 助手的进程边界加了大小上限。恢复出厂现在也会清掉 AI 在后台积累的全部数据;关掉 AI 助手后也会可靠地停下每一个后台索引与建议任务(包括活动中心工作台那几个),不再有任务在你关掉之后跑完、把一条已过期的建议写进去。

**面向开发者**

- **看似在后台的重活其实仍跑在主线程;取样定位到四处卡顿,现在都真正移出了主线程。** (1) 给 `async` 辅助函数标 `nonisolated` 在本工具链上**并不会**把它移出调用方 actor——`@MainActor` 调用方仍会在主线程跑完整个函数体,于是 `7zz` 列表解析卡在主线程(大归档约需 1.4s)。列表调用与 ZIP 中央目录加密探测现在用 `Task.detached` 包裹,这是唯一可靠 hop 到 cooperative pool 的手段。(2) 逐条目的 7-Zip / unzip 日期解析用 `DateFormatter`,首次解析会触发 ICU 加载 locale 符号表(冷启动数百毫秒)外加每次调用开销;现在按固定格式 `yyyy-MM-dd HH:mm:ss` 手动抽整数填进复用的 `Calendar`,彻底绕开 ICU。(3) 文件浏览器的 outline diff 用 `URL.standardizedFileURL` 建布局指纹,而它会对每个文件 `stat()`、且发生在**每次 SwiftUI reconcile**(约需 2.4s);现在改用纯路径字符串。(4) 菜单栏命令树会在**每次 FSEvents 触发(~120ms)**时重建,而其中一个启用状态 getter(`canDropIntoOpenArchive`)每次都 `fileManager.fileExists`——对打开的归档做一次 `stat()`,碰上慢速 / 断开的网络卷就挂死主线程(约需 2.5s);该 getter 不再碰文件系统(打开的归档已有 FSEvents watcher 盯着它是否消失)。ZIP 加密类型也改为打开归档时(off-main)检测一次、缓存进 session,解压 / 测试 / 打开条目 / 弹密码框都读缓存,不再各自重读整个中央目录(有的甚至读两遍)。这个加密探测现在也只读归档的中央目录(seek 到 EOCD 记录后只读那一段),而不是把整个文件 mmap 进来——在网络共享上(mmap 会退化成真实 I/O)探测一个几 GB 的 ZIP 不再把整包全拖过来。解压对话框复用打开时已加载的清单、解压前安全检查复用这批预载条目、解压后暂存扫描跑在后台,Finder /右键自动解压与一键创建浮窗的逐文件进度也和主窗口走同一个 ~80ms 的 `ProgressCoalescer` 合帧。
- **全面主线程审计消除了文件浏览器里所有剩余的逐帧 `stat()` 调用。** 第二轮分析(对所有热路径逐一审计,不只看取样命中的卡顿)发现了三处仍以 FSEvents 触发频率（~120ms/次）调用 `standardizedFileURL.path` 的位置,以及两处每次调用都现编译 ICU 正则的位置。`FileItem` 现在在后台构造时(在 `FileBrowserService` 中)一次性预计算并存储 `standardizedPath`,展开记忆查表、AI 组路径索引表、文件夹 key 计算等热路径全部改读纯 `String`,不再触发 `stat()`。文件夹模式 URL 的 `currentFolderKey` 改用 `.path`(来源于 open panel / FSEvents,已是规范路径)。基准测试解析器里整数匹配和时间值两个 `NSRegularExpression` 提升为 `nonisolated static let` 常量,ICU 只在进程生命期内编译一次模式串。`DiskImageBackend.extract` 的同步 `copyContents` 循环(目录列举 + 逐文件拷贝)改为 `Task.detached` 派发,防止从 `@MainActor` 上下文调用时阻塞主 actor。
- **`7zz` 列表改为流式解析(不再整串缓冲),解压后合并的逐项 stat 也移出主 actor。** 打开超大归档过去会同时持有完整的 `7zz l -slt` 输出串、它的 `\r` 替换副本、以及解析出的条目数组——几百万条目时是好几 GB,足以被系统杀掉进程。现在列表**逐 chunk** 喂进增量 `SevenZipListStreamParser`(逐行去 `\r`、随手产出条目),原始输出串和它的副本根本不再成形(整串入口 `parseSevenZipList` 委托给同一个 parser,所以原有 fixture 仍覆盖解析逻辑,另加逐字符分块测试守住流式边界)。解压合并的逐项 `resourceValues` / `fileExists`(可能落在慢速网络目标上)现在跑在后台任务里,而不是 `@MainActor` coordinator 上;符号链接逃逸的 containment 校验则有意保留在主 actor 不变。

### 改进 (improvements)

**面向用户**

- **发布版现在经 Apple Developer ID 签名并完成公证。** 由发布流水线打出的版本会用 Apple Developer ID 证书做代码签名、并提交 Apple 公证——随包的 `7zz` / `rar` 后端和整个 app 都以 hardened runtime 签名,公证票还会「钉」进 DMG——所以一台干净的 Mac 双击即可打开,不再弹「无法验证开发者 / 无法检查是否包含恶意软件」的 Gatekeeper 警告,离线也认。又因为发布版现在带上了通过校验的 Apple Team Identifier,它的「快捷指令与 Siri」动作也能正常工作,不再被标记为不可用。
- **「查找包含文件的归档」现在即时又可预期。** 它不再先让端上模型把你的措辞猜成关键词再搜——而是直接拿你输入的字去匹配你打开过的归档里的文件名,所以更快,而且关掉 AI 助手也一样能用(它现在就是一次对本机清单缓存的搜索)。结果列表也给滚动条留了位置,不再盖住「打开」按钮。
- **AI 设置可在 Spotlight 里搜到。** 后台 AI 索引与静默索引设置现在会出现在 Spotlight 的设置搜索里、并直接跳到对应位置;其中涉及隐私的那些可以被搜到、打开,但和其它数据采集开关一样,不能被 Siri/Spotlight 直接开关。
- **浏览大归档与大文件夹更顺、更省内存。** 在大归档里逐层切目录不再每步重建目录树,搜索归档时每次刷新只过滤一遍(原来两遍),滚动大文件夹时同时取的图标数量有上限、不再一次性发起几百个。AI 助手也只在浏览器真要显示建议时才重建它的逐文件查表(并给失败解释缓存加帽),后台索引占用更小。

**面向开发者**

- **统一的端上 AI 数据层(更强但依旧隐私的 AI 的底座)。** SimpleZip 正在搭建一套共享、完整单测的底座来决定「怎么把数据喂给苹果端上模型」——定位是「本地事实索引 + 小模型智能组织层」:App 负责把本机事实采集、过滤、脱敏、按预算整理成结构化信封,每个信封都带明确的隐私描述块(端上执行、标准/深度本地上下文强度、确认排除的红线类别)和「省略说明」(哪些数据没给、为什么),外加可复用的证据卡和经校验的来源引用;模型只负责解释、命名、排序、归组、起草。模型返回的每个路径、任务 id、归档条目 id 都会和 App 实际提供的候选校验,任何危险动作一律回到原生确认流。口令、密钥材料、加密归档条目名、密文、解密明文绝不进入这一层——靠契约保证(确定性编码,发给模型的事实可单测、可导出审查)。(一个刻意「非 AI」的近亲活在安全层、而非这一层:密码输入框的上下文提示只给低敏的回忆线索——文件名 / 目录 token、来源域名——并硬性拒绝任何看起来像真实口令的字符串,绝不读剪贴板、绝不自动尝试、永远需要手动点击。)信封之上还有一层确定性的「组织」基元,模型只做精修:归档画像与角色识别、一套受控语义标签词表(模型只能从中选取并重排,带确定性分数、证据与用户纠错降权)、失败诊断标签与按标签的修复手册、低敏位置上下文、按任务/按归档的 AI 索引、文件类型分类与文件夹画像(角色 + 推荐视角派生)、从 marker 文件嗅探依赖生态(Node / SwiftPM / Python / Rust / Go / Java / …)且把 `.npmrc` / `.env` 这类敏感配置只记「存在」、绝不读内容、可保存的查询计划、把已索引对象重新归组的「视角」(发布/源码/失败/签名/清理)、直观呈现归档内部构成的「内部地图」、一份模板化的发布前 checklist(测试 / 核对哈希 / 检查签名 / 打开报告——状态由确定性事实填充、GPG 关闭时签名项不出现、模型只负责组织)、版本关系标注(同内容/新构建/源码vs二进制/分卷/旧备份/本地化变体)、乱码文件名的确定性候选编码评分(把 raw bytes 用 Shift-JIS / EUC / GBK / Big5 / CP437 / UTF-8 解码并按脚本一致性排序,模型只在候选间重排、绝不发明文件名;加密条目排除)、确定性的下一步动作排序器、一道打扰阈值门(决定 AI 能多吵:安静→状态→内联速览→建议卡,让无害的 Finder 双击保持安静、自动解压绝不被打断)、把模糊搜索重写成可执行本地查询、一个基于 JSON AST 的上下文压缩器(在模型读取前把反复出现的长 key/value 换成短 token 以降低首字延迟、输出时还原——绝不用正则替换、小于 1.5 KB 不压缩)、虚拟 AI 工作区、带硬性默认排除规则(系统/密钥/缓存/开发依赖/临时解密目录)的白名单后台预读作用域,隐式的兴趣/反馈信号、按时段感知的启动目录排序器,全局建议总线契约(每个界面位置发请求→拿回经校验、可忽略的建议卡),确定性的「为什么没有推荐」解释器,一套失败降级矩阵(每种 AI 失败——模型不可用、JSON 坏、引用无效、危险动作、隐私阻断——都映射到确定性兜底与调试分类,让 AI 永不成为唯一数据源、失败也绝不用 alert 打断用户),以及一套能力分层的引擎协商契约(确定性/端上/高级可选)——让 AI 界面常驻、只有模型增强受门控——创建/解压 Auto-Tune 安全闸(逐字段策略表,让模型预填表单选项,但绝不自动改密码、GPG 材料、加密强度、目标路径或破坏性整理动作,碰加密内容的改动直接丢弃,也绝不覆盖用户已手改的字段),以及确定性的创建/解压建议卡规则引擎(把 dry-run / 预检事实直接变成可点击的卡片——改名输出、查看安全报告、去单根目录、跳过符号链接——只携带稳定规则 id、证据 token 和白名单内动作,无模型也工作,模型发明的动作一律剔除),以及确定性的对比解释器(把已算好的归档 / 发布 diff——新增 / 删除 / 修改与 hash 变化的条目数,外加「新增校验文件、签名或 app bundle」这类发布信号——直接变成稳定的提醒与下一步动作 id,如 looks-like-release-build / release-notes-may-need-sync / 跑发布检查 / 起草发布正文;模型只写一句话摘要,白名单外的 id 一律剔除),以及确定性的预设推荐器(把输入画像——角色、扩展名、marker 文件、是否媒体为主——映射到一个预设类别和一组「安全」option 提示,如排除垃圾 / 创建后测试 / 可复现 / store 级压缩,并在用户已有的预设里匹配;密码、加密、GPG 选项永远不可被提示——只能用户手设——安全白名单外的提示一律剔除),以及带硬下限的安全关注摘要(App 的确定性扫描据风险 hint——路径逃逸、可执行、符号链接等——定一个最低关注级别:无 / 提示 / 谨慎 / 先别解压;模型可以把措辞写得更准,但一道覆盖所有路径的钳制禁止它**下调**级别、在「先别解压」级强制主动作=查看安全报告,绝不让温和的措辞把用户哄过一个真实警告),以及自然语言选择器(把「选中所有像发布产物但还没测试过的包」改写成结构化过滤——受控语义标签、扩展名、关键词和任务状态——由 App 在自己的索引上执行来高亮或圈定条目;模型只返回过滤条件、绝不返回路径,受控词表外的标签一律剔除,空过滤选中零项而非全选),以及设置医生(把当前设置状态——AI 开关、后台活跃度档位、归档预读、文件夹预索引、Spotlight 索引——与用户陈述的意图合并,一句话作答,只建议「对当前状态真正适用」的动作,no-op 或发明的动作一律剔除;每条建议都是 AI / 后台 / Spotlight 偏好或「打开某设置页」——绝不改加密、密码或 GPG——且不自动应用,回到用户确认的原生设置页),以及任务 / 批处理规划器(把「把这个目录里像发布包的东西都检查一下」变成一串有序步骤——或把多选项分组——动作只能取自固定的 action catalog;模型绝不输出 shell,步骤动作与分组动作都走白名单,分组引用的每个 itemID 都对照真实选择项校验、模型无法凭空捏造,「需要用户审阅」是模型关不掉的常量 true,所以计划永远先展示给用户确认、再调用现有动作),以及场景路由器(让 AI 像一个整体而非一堆分散按钮:一句「这个包有什么问题」会被——按当前 surface 与选择确定性地——路由到对的 AI 场景:活动筛选、归档搜索、操作预览、失败解释、设置助手、工作区、动作推荐或报告解释;模型只在低置信度时辅助,它选的目标必须是 App 当前提供的、否则退回确定性路由,confidence 钳到真实的 0–1),以及一套后台调度规则,从纯运行时快照判定低负载 AI 现在最深能跑到哪一档(无 → 确定性索引 → 模型预热 → 深度上下文)——启动后第一分钟内不跑、有重归档任务在跑时不跑,低电或省电模式只允许只读索引,模型任务要等用户空闲,最深档需「充电 + 空闲」——让助手随时保持热度,却绝不与用户的工作抢电和 CPU),以及一张数据生命周期策略表,让保留与清空可预测、可审计(每类 AI 数据都带保留策略——prompt facts 永不持久化,调试记录最多 20 条且只留一天,派生索引跟随归档缓存,反馈 30 天滚动、习惯摘要 90 天;关闭某个数据开关会映射成 builder 记录的稳定「用户已关闭」省略说明;每个清空入口都明确声明它级联清掉哪些派生类别——绝不碰任何真实文件、任务或归档),以及现在复用到文件/归档建议面的端上「文件事实 + 虚拟文件夹」层:一份统一的文件事实(完整当前路径,加 POSIX 权限 / owner / 默认与候选打开 App / 角色标签 / 同目录失败组 / 项目根提示),配一道确定性的内容可读性门控——无读权限、敏感目录(`.ssh` / `.gnupg` / keychain)、临时解密目录、疑似密钥文件名、用户排除的文件一律挡住内容读取并记下原因;一组受控的只读数据增强动作(哈希 / 测试归档 / 刷新清单 / 结构指纹 / 刷新文件事实 / 刷新默认打开方式),模型只能「建议」——对照候选集校验,加密 / 无读权限 / 被排除的输入一律拒绝,且只产派生信号、绝不产文件内容;一棵可持久化的虚拟文件夹树,内容区经显示前清洗渲染——丢弃发明引用与不安全节点,并剥离任何「破坏性」主动作,让删盘永远不可能是一键动作;动作词表扩到覆盖 SimpleZip 原生工具入口(哈希 / 创建 / 测试 / 转换 / 发布检查 / 用某 App 打开)和 AI 文件夹自身的虚拟管理(合并 / 拆分 / 移动节点 / 移除引用 / 加种子 / 删工作区),外加真实硬盘的复制与删除,每个动作都带按 case 的安全姿态,任何写盘或启动任务都需确认、删盘则是「破坏性 + 强确认」(绝不自动、绝不当主动作);一个工作区候选池,产出证据缺口卡(每张都绑定它的补救增强动作)、确定性的「为什么被抑制」原因,以及基于 Jaccard 重叠的合并候选;一份用户「AI 文件夹种子」(主题提示词、固定与排除的引用,AI 据此再派生,而非静态收藏夹),拆分组对照候选池校验;一个工具交互终局,按「接受 vs 撤销 patch」和终局结果而非停留时长来给创建 / 解压反馈打分(让一次快速而正确的解压读作正反馈而非忽略),外加一个只保留加密条目计数的归档打开会话;以及一个主窗口「AI 建议层」surface——它唯一地禁止确定性兜底,所以只会显示真实的模型输出;一组把红线从「调用方自觉」搬进类型系统的硬边界(上下文信封只能经一个会抛错的工厂构造、拒绝红线负载;source-ref 身份是 64 位稳定 id,配一个拒绝空引用的校验器和候选集内碰撞登记表;上下文压缩器是 field-aware 的,所以一个恰好叫 `mv` 的用户文件绝不会被错写成诊断 token、token 撞车则退回不压缩;日志取尾在截断前先扫私钥块、宁可扣留也不泄漏被切断的密钥;敏感目录判定按 path component 匹配,所以目录本体——不只是它的子文件——也被挡,临时解密目录判定只在真正的系统 temp 根下触发;文件事实只经一个默认省略绝对路径的投影进入 prompt);一个逐 surface 的交互信号层,把轻量的 曝光 / 展开 / 点击 / 忽略 信号确定性地折叠成 7 天 / 30 天计数和 interaction 亲和度(证据 token 入库前脱敏、快速关闭绝不被误判为反馈);以及一个确定性后台计划器,把当前运行时档位、证据缺口、陈旧工作区 / surface 和用户反复关注的东西,变成一组按档位门控、绝不超预算、可取消的预热 job——每一个都是纯值类型,无模型也能工作,且全部可单测,如今真正接通执行:计划器读取真实工作区证据缺口(成员还没算过校验和、小归档还没测过完整性)、把哈希与完整性测试 job 喂进既有插电门控后台 pending 检查流水、空闲时段据此补齐(其余 job 类型先排在后面);外加保留为可复用内部地基的 AI 文件夹后半段:一个跨位置语义聚类器(在共享名字 token——含 CJK 子串——和共享任务 / 归档血缘上做连通分量,位置绝不决定成员资格、主题身份取自成员而非任何路径),一个衰减抑制账本(一次「不感兴趣」是一条永久、随时间衰减、多次更狠的降权,且模糊匹配——加一个文件改了指纹也躲不掉,强主题衰减后还能重新浮现),一份薄虚拟文件夹 plan 加一个确定性树构建器(模型只给目录起名、把候选分到组里;App 推导每个节点动作并跑显示前清洗),一条多源发现流水线(文件索引 + 任务记录 → 候选 → 聚类 → 质量与数量策略 → 抑制),一个持久文件记忆索引容器,一套 AI 加权的工作区排序(主题强度 + 打开频率 + 衰减最近度,点一下是轻推而非钉死、常用的弱文件夹会随时间升上来),一条只推荐完整、够分量主题且限制数量的推荐策略,一条把你的调教——你留下的成员、你移除的成员、你偏好的种类,以及你给它的分组起的名字(连同这些成员在哪儿)——直接回喂端上模型的学习闭环,让每个文件夹朝你的口味生长、而不是每轮从零重排,以及一个 opt-in、仅白名单的后台文件索引器——只读、仅元数据、按预算受限、空闲调度,且经对抗加固防越权逃逸(符号链接、网络卷、`..`、隐藏与敏感目录)和凭据泄漏(疑似密钥文件名与凭据目录在被索引前就丢弃)。
- **开发者工具可以实时查看 AI 能看到的本机数据。** 隐藏 DevTools 面板的只读「AI 可用数据」现在每秒刷新一次,显示助手就绪、归档记忆规模、Spotlight 捐献计数、后台索引 scope/文件/画像数、内容摘要与建议 token 计数、门控状态、各 pass 候选诊断、反馈/pending 检查计数,并能复制 AI 索引和完整 Spotlight 捐献数据集。加密内容、密码和密钥仍不在这里,因为它们从不进入 AI。
- **AI 建议流水线有了结构化缓存和按电源门控的只读检查队列。** 建议现在使用带 payload 的结构化动作、逐 token 抑制与反馈、AI 排序的文件/归档清单预读、缓存文件组,以及三阶段 pending 检查队列:先记录哈希/测试任务,再存到 app 侧,之后在电源门控允许时执行安全只读检查。
- **AI 侧栏实验从正式 UI 中收回。** 可复用的虚拟文件夹组件、排序、复核、主题边界学习、反馈和基准测试仍留在内部地基里,但 1.0.0 实际可见的界面是文件/归档抽屉建议,不是独立侧栏入口。
- **主工具栏动作推荐有了可测试的 provider 接缝。** 上下文工具栏原有的两个推荐按钮现在来自确定性的 `ContextualToolbarActionProvider` 快照,并可接入下一步动作排序器;可见按钮和实际调用的 model 方法保持行为等价。这是白皮书动态推荐引擎在 App 侧的第一步,不会改变主窗口工作流。
- **AI 工作区 token 提取现在能正确处理版本号。** 把文件显示名转换为语义 token 集合的函数原先逐字符扫描、在每个非字母/数字字符处断开,所以「1.0.0」会产生「0」「4」「5」三个单字符片段——全都低于最短 token 长度、全部被丢弃。现在补充一个「按连字符/下划线/空格整段保留」的切割过程,「SimpleZip-1.0.0-macos-arm64.dmg」由此能正确提取出五字符 token「1.0.0」。该修复使发布工作流基准测试中候选池的强 token 覆盖率(专注律)从 80% 提升至 100%。
- **AI 工作区 prompt 事实可携带更多意图 token。** `AIWorkspacePromptFact` 初始化器此前把每个工作区 prompt 信封里的 token 列表上限设为 12 条;由宽查询计划(语义标签、任务标签、关键词合并量多)构建的工作区会把低优先级的意图 token 静默丢弃。上限提高到 14,确保典型发布工作流工作区在覆盖全部语义 token 和关键词 token 的基础上,还能覆盖其四个任务标签意图 token。
- **端上 AI 正在迁出主程序、改到独立进程跑。** SimpleZip 的 AI 推理与后台索引正从主程序里抽出来:前台请求走内嵌的 XPC Service(按需拉起,不是登录项、也不受「允许后台」门控),后台索引走一个专门的 agent —— 即便 App 关着,launchd 也能按计划周期把它唤醒。共享的扫描、索引与内容预读代码现已下沉到 `SimpleZipCore`,由 App 与 agent 复用而非各拷一份 —— 同一份扫描编排、只走白名单的只读/预算受限元数据遍历、内容预读脱敏,以及「最久没扫优先」的渐进覆盖,在哪个进程里都是同一套。App 通过带版本的配置文件把 AI 设置同步给 agent,AI 主开关作为硬红线强制(主开关关 → agent 拒绝一切生成);agent 读白名单、跑一轮预算受限的扫描、把记录写回共享派生数据,并据配置间隔自节流,带单次运行时长上限(超预算的目录留待下次唤醒续上),并以 `ProcessType=Background` 交给系统做电源感知的延迟调度。为此声明了一个周期后台索引 LaunchAgent(默认关闭,仅在开启「静默后台索引」时激活);dev 与正式构建用各自隔离的 bundle id、Mach 服务名与 LaunchAgent 标签,确保自签 dev 版与已发布版永不冲突;内嵌 agent helper 也按 app 的 bundle-id 命名空间签名(而非默认的裸工具名),与 app、XPC service 命名一致。每个端上模型 pass 现在都完全在那个引擎进程里、经统一的 `generate(kind:)` XPC 契约跑 —— 报告与解释的散文、文件行/归档行的建议、AI 文件夹的 plan / review / 分组 / 核查不扣题,以及自然语言的归档与设置查询 —— 主程序二进制因此不再创建任何语言模型 session,且每个 pass 的拼 prompt / 解析 / 校验都在共享引擎层,后台 agent 无需拉起主程序即可跑。连只读的「模型是否可用」查询现在也走引擎(经 XPC,结果缓存以便 UI 同步读取),主程序二进制因此**完全不再 import 端上模型框架**。

- **开发者工具:端上 AI 引擎 + 后台调度监视器。** 隐藏的 AI 数据实验室新增一组只读探针,观测迁出主进程后的管线:统一 `generate(kind:)` 契约自检、被动引擎 pass 计数(引擎自启动以来跑过哪些 pass、多少次、成不成)、后台调度预览(据当前运行时与信号 `AIBackgroundPlanner` 会规划哪些 tier-gated job)、工作区证据缺口读数(哪些工作区成员还缺校验和 / 归档完整性测试 —— 正是驱动调度器后台补哈希 / 补测的输入)、pending 检查队列读数(下游 —— 待插电执行的检查与已完成数)、交互 / 兴趣摘要读数(调度器另两个输入 —— 跨表面信号计数与归档打开位置亲和,印证兴趣收集真喂进调度)。全部确定性、瞬回、不碰模型,结果常驻可复制状态卡。
- **端上 AI pass 现在把 prompt 裁进模型固定的上下文窗口。** 端上模型上下文大小固定,prompt 一旦超限,该 pass 会直接报错、静默返回空。每个会喂模型「列表或长文本」的 pass —— 归档条目、文件 / 文件夹 / 工作区建议候选、活动聚类与 chip、自然语言设置目录,以及报告 / 摘要 / 文件描述散文 —— 现在都按一个保守、CJK 感知的预算裁每条长度和 prompt 总量,让超大归档、超长文件或完整本地化的设置目录退化成「裁过但仍能跑」的 prompt,而不是溢出后空手而归。(由 DevTools 引擎 pass 计数照出 —— 它逮到两个后台 pass 在静默溢出。)
...
- **只读镜像 / 容器格式也能从 Finder 关联打开了。** .iso、.cab、.cpio、.xar、.pkg 现在把 SimpleZip 注册为处理器(出现在「打开方式」里),并补进设置 → 文件关联列表。这些是 Apple 系统/系统默认应用拥有的类型,SimpleZip 做「备选打开方式」、不抢默认。
- **损坏归档数据救援。** 右键损坏的归档 → 尝试数据救援：7-Zip 在 CRC 错误后继续解、ZIP 中央目录损坏时扫 local header——所有还能读的内容落进新的「原名 (rescued)」目录（绝不覆盖任何东西；原包永不被改动）。报告列出救出多少文件、哪些条目读不出来，并带固定免责声明：救出的文件可能不完整，归档本身不会被修复。安全检查丝毫不放松——损坏包正是敌意输入的高发形态，解压前过路径校验、落盘后扫逃逸符号链接（不安全的产物直接删除并硬失败）。
- **归档批量体检。** 右键选中的归档（或一个文件夹，扫它的顶层）→ 批量体检：每个归档一次过完——打开、完整性测试、可疑路径、macOS 垃圾、加密条目、缺失分卷、只读格式，以及整批内结构相同的疑似重复包。密码只从会话缓存静默尝试；解不开的如实标「需要密码」，绝不弹窗。单包失败不会中断整批；逐包结果留在任务里，报告可从活动中心重开。
- **直观的安全评分（A / B / C）。** 路径安全报告现在最上面先给一个风险等级——A（低）、B（中）、C（高）——无需逐项阅读发现清单即可了解归档安全性。纯规则系统（不用 AI 判安全）：每条发现加上加密、macOS 垃圾、缺分卷、损坏都按严重度加权，**按发现的最严重那一项**定级——所以轻微提示不会被放大成「高风险」，而加密（本就不是风险）也不会拉低等级。评分一并写进导出的 Markdown / JSON 报告。
- **快速核对发布组。** 右键文件夹 → 快速核对发布组：只看文件名、瞬时判断这个文件夹是不是一个完整、可校验的发布——可下载产物（.dmg / .zip）或签名 .szs、SHA256SUMS、公钥、VERIFY.md。它只读文件清单（不算哈希、不验签），所以是瞬时的；结果用与「检查发布目录」完全相同的报告呈现，逐项标「已包含 / 缺失」，并提示要真正核对哈希与签名就走完整检查。
- **检查保存的发布路径是否还在。** 设置 → 自动化 新增「发布路径健康」：点「立即检查」，它会过一遍每个工作区预设指向的文件夹、以及发布历史里的每个产物，把文件被移动 / 删除后已丢失或读不到的标出来，还在磁盘上的给「在 Finder 中显示」。
- **长任务完成时可选发系统通知。** 活动中心设置新增「长任务完成时发通知」开关（默认关）：当 SimpleZip 不是当前 app、且一个跑了至少十秒的任务完成时，你会收到系统通知——可以丢一个大压缩 / 解压在后台，切走干别的，完成时被告知。只在你打开这个开关时才请求通知权限，该设置随设置备份迁移。
- **已打开的归档在磁盘上被改动时给提示。** 浏览一个归档时，SimpleZip 现在会盯住这个文件本身（用 file presenter，不是轮询文件夹），被别的 App 移动 / 删除 / 改写时弹横幅——「已被 SimpleZip 之外改动」带「重新载入」按钮，或文件没了时「已被移动或删除」。它跟列表那一刻记下的磁盘状态比对，只在真有外部改动时才提示（且横幅绝不碰那条必须保持安静的文件夹刷新路径）。
- **找出归档里的敏感与配置文件。** 归档菜单 → 查找敏感与配置文件:对打开的归档条目做一次快速的、按文件名的扫描,把值得注意的归类——看起来像私钥/凭据/机密的文件(`.pem`、`.key`、`id_rsa`、`.env`、名字含「secret」「token」等)、许可证与版权文件、配置(`.yaml`、`.json`、`.conf`、`Dockerfile` 等)和脚本。纯确定性、只按文件名匹配——从不检查内容,加密条目(列不出名字)也永不出现。macOS 26 上有个「解释」按钮,把扫描结果写成一段说明,并提醒疑似机密类别勿随意外传——AI 只解释确定性结果,从不扫列表、不做判定。
- **找出归档里的近似重复文件。** 归档菜单 → 查找近似重复文件:在已有的「字节完全相同」重复查找之外,这个把文件名归一化(剥掉「副本 / (1) / _v2 / final」这类标记)后落到同一个的归为一组——于是 `report.docx`、`report (1).docx`、`report_v2.docx`、`报告 副本.docx` 会进同一组——并按校验和把每组标成「字节相同的副本」或「相似的版本」。只按文件名分组、从不比较内容;加密条目(列不出名字)排除。macOS 26 上有「解释」按钮总结哪些组值得细看——只读,不建议删除。
- **归档元数据报告。** 在打开的归档里右键 → 元数据报告:条目列表从不显示的归档级头部信息（格式、物理/头部大小、方法、固实、块数、分卷数、注释），加上条目聚合——压缩方法分布、加密条目数、AppleDouble/__MACOSX 痕迹、属性分布，以及安全报告发现计数的指引。只读；走统一报告导出菜单。
- **打开性能报告。** 在打开的归档里右键 → 复制打开性能报告：一张小 Markdown 表——后端列目录耗时、界面建树耗时、条目数与格式，计时里含密码弹窗等待时会如实标注。纯剪贴板设计；浏览过程不渲染任何相关内容。
...
- **活动中心「临时工作区」页。** 显示加密临时卷是否挂载(及挂载点)、解释其一次性密钥/退出即毁的生命周期，列出系统临时目录里的 SimpleZip 条目与总占用，并提供「立即清理」——只删上次会话留下的陈旧条目，绝不碰本次会话正在使用的 staging 目录。
- **整个任务队列可以一键暂停 / 恢复。** 活动中心新增「暂停队列」按钮：冻结所有可暂停的运行中任务（解压、创建、测试、转换、比较、基准），新的重任务排队等待直到恢复；你单独手动暂停的任务在队列恢复时保持暂停。无法冻结的任务（哈希、拆分合并、文件操作）会自行跑完——头部文案如实说明，不假装全冻住。暂停态仅本次会话有效，重启不保留。
- **报告可从活动中心重新打开、且跨重启保留。** 发布检查、.app 专项检查、发布目录检查、元数据报告、空间分析任务完成后，卡片上有「查看报告」按钮，不必为重看报告而重新运行；比较结果与发布检查/元数据报告随任务历史落盘，重启后照常可看（超大比较跳过持久化以免历史膨胀）。
- **活动中心来源筛选与清理。** 新的来源筛选（应用内 / 命令行 / 快捷指令 / URL scheme / Finder 服务）与现有状态筛选正交组合；侧栏行显示红色失败计数胶囊；「仅清除成功记录」清空历史但保住失败项；创建类任务完成后新增「以新参数重跑…」——重开创建对话框并预填原请求。
- **用一句话切出任务的临时视图（macOS 26）。** 当 Apple Intelligence 可用时，活动中心头部有个 sparkles 按钮，输入一句自然语言描述——「minecraft.zip」「今天失败的」「30 分钟内」「命令行跑的」——本机 AI 把它抽成精确条件（关键词、时间上限、状态、来源），再由 app 在**代码里精确匹配**:「zip」必中所有名字含 zip 的任务、「30 分钟内」必留所有 ≤30 分钟的任务,确定性的、每次都准。（模型只负责理解你的话,从不亲自扫列表选行,所以匹配可靠、不会编造结果。）筛选生效时按钮填充紫高亮，点一下即清空；若这句话抽不出任何条件,按钮不点亮并给一行提示。每个任务卡也在状态同一排显示运行时间（如「2 小时前」「昨天」）。AI 助手关闭或不可用时整体隐藏。
- **`.siz` 全面接入归档工具。** 测试、批量体检、比较、查重、空间分析、发布检查都接受 `.siz`，跑在它解包出的内层归档上；发布检查的 SHA 和公钥同捆检查仍对 `.siz` 本身——那才是你分发的文件。数据救援把损坏的 `.siz` 当作它本来的 tar 容器整体救出。右键菜单、「归档」菜单与文件夹筛选都把 `.siz` 当成真正的归档。
- **最近的发布包与活动中心任务出现在 Spotlight 里。** 发布助手记录的每个发布按版本、日期、产物文件名、SHA-256 前缀索引进 Spotlight（macOS 15+），每个收尾的活动中心任务按标题、操作、来源、状态索引——搜版本号、文件名、或「失败的解压」都能直接找到。现在点结果会带你过去——任务会打开活动中心、跳到它所在分类并滚动高亮那张卡，发布包则在 Finder 里显示它的产物。只索引这些可公开的元数据；结构指纹与加密归档的内容绝不索引。每次启动重新同步（发布另在每次跑完后、任务另在每条收尾时）；清空或收紧活动中心历史时，那些任务也会从 Spotlight 一并移除——不留任何陈旧残项；macOS 14 及更早直接跳过。设置→自动化里有开关可关闭索引（并清空已索引内容），想更私密就关掉。
- **用一句话找设置（macOS 26）。** 当 Apple Intelligence 可用时，设置侧栏顶部有个小 AI 搜索框：用自然语言输入你想要的——「删除前别再问了」「让搜索更快」「把 GPG 那些关掉」——SimpleZip 就跳到对应设置（切到那一页、高亮那一行），任意语言都行。如果你明确说要开或关、且它是安全的便利开关，会执行切换；安全敏感的设置绝不这样切（与语音控制同一套三道闸），只导航过去。模型只负责挑你指的是目录里哪一项——绝不改动 app 没明确放行的东西。AI 助手关闭或不可用时整体隐藏。
- **从 Spotlight 跳到某个设置。** 在 Spotlight 里搜设置名（如「Spotlight 索引」「归档内容缓存」「创建后验证」「显示隐藏文件」「行密度」），点结果就直接落到那一项——SimpleZip 会打开设置、切到对应分页并高亮那一行。各分页里几乎每个选项都覆盖了。只索引设置名、不索引设置值，受同一个 Spotlight 索引开关管。覆盖每一个面板——通用、归档（安全、后端、压缩默认值）、浏览、视图（分组、隐藏后缀）、文件关联、GPG（签名、智能卡）、软件更新、自动化（命令行、快捷指令、路径健康）、健康与备份；活动中心自己的设置（任务并发、完成通知/提示音、失败时弹出）和临时工作区也能搜到——点了会打开活动中心并切到对应分页。只索引设置名、不索引设置值，受同一个 Spotlight 索引开关管。设置 → 自动化 → Spotlight 还会显示大概索引了多少条（归档、文件、设置），并有一个「重建」按钮——清空并重新捐献全部内容，搜索结果不全或过时时用得上。
- **用语音开关安全设置——不用打开 App。** 对 Siri 说（或运行「更改设置」快捷指令）就能翻转一个便利类设置——「打开 Spotlight 索引」「关闭自动解压」——SimpleZip 在后台改好、不弹窗；正开着的设置窗口会实时跟着变。只有安全、无破坏性的开关能这样改：涉及安全的设置（删除确认、GPG、预设密码、是否允许自动化复用预设密码、归档路径安全策略）刻意永远不可语音控制，并用三道闸焊死，精心构造的请求也碰不到它们。
- **macOS 26 交互式设置开关（快捷指令 / Siri）。** 在 macOS 26 上，「设置开关」动作会把一个 SimpleZip 设置以交互式卡片弹出——卡片里有一个真正的开关，点一下就地翻转、卡片刷新成新状态，不打开 App、不弹窗。从快捷指令 App 或 Siri 运行它；走和语音控制同一份安全开关白名单（同样的三道闸），涉及安全的设置永远不会出现。（macOS 不允许 App 往 Spotlight 搜索结果行里塞一个真开关；macOS 25 及更早也没有交互式卡片——语音开关和点结果跳到设置覆盖那些系统。）
- **创建与解压对话框的 AI 速览（macOS 26）。** 打开任一对话框、Apple Intelligence 可用时，预检概要下方自动出现一段 AI 速览——无需点按钮。**创建速览是实时的**（要打包的是你当下任意挑的文件，没法提前算）：给出大致耗时感受和一条基于真实顶层条目（而非只看计数）的实用提醒——能看到顶层被打包的真实文件/文件夹（于是能提醒「把文件夹塞进 gzip 这种单文件格式会先打成 tar」或同名冲突），**现在还会参考后台助手对这些文件已经「知道」的信息（角色标签 / 一句话摘要）以及你最常用的压缩设置**，让提醒更贴合你的内容与习惯（例如这次压缩级别和你平时不一样时）；随你改格式、级别实时更新。**解压速览改为预烘焙、瞬间出现**：归档是稳定文件，后台助手会提前把「这是什么包 / 解出来大致是什么」的一句话定性烘好（你正解压一个它还没见过的包时，前台也会按需即时烘一次并永久缓存），于是再开同一个包速览立刻就在，不再像以前每次开窗都干等本机模型现跑（还偶发抽风）；可疑路径、会被覆盖的文件、缺失分卷、磁盘空间不足仍由上方那套确定性预检照常即时提示。每条提醒都严格基于事实——不会猜测未看到的文件类型、不会编造数字、不会超出本机模型的上下文上限；全程本地推理，只描述/建议，不会修改设置、不会跳过安全询问。一道很薄的分割线把它与确定性预检统计分开。AI 助手关闭或不可用时整体隐藏。
- **本地 AI 报告助手（macOS 26）。** 当 Apple Intelligence 可用时，几乎每个报告上都有一个只读的「解释」助手：发布检查报告的「总结风险」、路径安全报告的「解释等级」、.szs/.siz 验签结果解释（签名是否有效可信、文件是否都与签名清单一致）、可复现构建报告、归档元数据（「这是什么样的归档」）、空间占用、数据救援结果、发布目录检查——再加上失败任务卡上的「解释失败」、批量体检后的「建议标签」、以及「复制为 Issue」旁的 AI 润色 Issue 草稿。每个助手都吃到报告背后的具体细节——真实的示例条目（被标记的路径、垃圾文件名）、文件数与体积、更完整的命令日志，而不只是界面上展示的汇总计数，这样输出会指出具体的东西而非空泛套话。每个都把结果落进可编辑文本框（可用 Writing Tools 改写）供你审阅、复制——助手只读取与解释，不会改动任何文件、设置或任务，输入也不包含加密归档的内容或任何口令。全程本地推理。可在设置→自动化里关闭；旧系统或模型不可用时整个入口隐藏。
- **压缩默认值能从你实际的压缩习惯里学。** SimpleZip 现在会记录你创建归档时选的压缩选项——本地、只记非敏感旋钮（压缩率、更新/路径模式、跳过垃圾等），绝不记口令或任何加密内容。在 设置 → 压缩 里编辑某格式的默认值时，多一个「按我最常用的来」按钮，把每一行填成你对这个格式最常选的值（保存前仍可改）。创建对话框里，当你还没为该格式保存默认值时，也会出现一键「套用你常用的设置」，把你最常选的选项直接填进来。记录默认开，可在同一页关掉。**开启端上 AI 后，这个按钮变成「套用推荐设置」**——不只看你的习惯，还据你这次要打包的内容(媒体多 / 源码 / 发布物)叠加安全建议(如媒体多→store 级别、发布物→可复现),与「套用常用设置」按 AI 开关**二选一**;密码 / 加密 / GPG / 目标路径绝不被预填。
- **解压对话框也能学你常用的解压设置。** SimpleZip 现在会记住你实际选的解压行为开关——本地、只记非敏感项（跳过垃圾、跳过符号链接、解到同名子文件夹、冲突自动重命名、完成后显示、原包移废纸篓），绝不记口令或目标位置。当你的常用选择跟当前所选不同，解压对话框会在「套用预设」旁多出一键 **用你常用的** 按钮把它们填上。只有你亲手确认的对话框才会学——从 Finder 打开时的自动解压绝不学——默认开，可在 设置 → 通用 里关掉。**开启端上 AI 后，这个按钮变成「套用推荐设置」**——习惯打底 + 据当前归档的 preflight(有符号链接→跳过、会覆盖→自动改名、单层壳→去壳、顶层散乱→解到子文件夹)叠加安全建议,与「用你常用的」按 AI 开关**二选一**;目标 / 口令 / GPG 绝不被内容推断预填。
- **创建对话框给压缩大小预估。** 压缩前预检概要现在会显示归档大概多大：SimpleZip 抽取文件样本、用与所选格式相近的算法压一遍再按比例外推，带 ≈ 和占原始大小的百分比。换格式或级别会更新，全程后台跑，并明确标为预估——实际大小取决于你的文件。
- **在 Spotlight 或 Siri 里直接找到压缩包**里**的文件。** 打开归档时，它里面的非加密条目会被记在本地，之后你在 Spotlight 里输入一个文档名，**那个文件本身**就会出现，下方小字标明它在哪个压缩包里。点开 → SimpleZip 打开那个归档并**直接跳到这个文件**——选中、滚动到可见、地址栏也跟着走到它所在的包内路径；如果你开了「从 Finder 打开时自动解压」，则只解压这一个文件并在 Finder 里显示它（不是解压整个包）。你仍然可以问 Siri、或跑「查找含某文件的归档」快捷指令拿到含某名字的归档——macOS 26 上还有个应用内版本:归档菜单的 **查找包含文件的归档…**（Apple Intelligence 可用时）会打开一个小框，用自然语言描述你要找的文件（「我的简历在哪」），本机 AI 抽出文件名关键词，SimpleZip 就列出打开过的、含该文件的归档——点一个就打开。AI 只负责抽搜索词，真正的匹配还是那套确定性缓存查询，且只在非加密名字上做。设置 → 自动化 → 归档内容缓存让你掌控这份记忆：可以关掉（关掉即清空）、设置记住多少个归档、过多少天后忘记、查看当前占用、随时清空。隐私优先——加密条目永不记录也永不索引（整包加密的归档直接跳过），从别的归档里打开的归档不记录，每个归档索引的文件数有上限，缓存不进设置备份，恢复出厂时清空。捐献进系统 Spotlight 索引还要遵循 Spotlight 索引开关，关掉任一个都会全部移除。

**面向开发者**

- **统一报告导出底座 + 各报告全面接入。** 比较、发布检查、安全、空间分析、`.szs` 验证报告统一接进「导出报告」下拉菜单（与创建对话框「套用模板」同款形态）：复制摘要 / 复制为 GitHub Issue（带可直接粘贴的环境表）/ 导出 Markdown / 导出 JSON，全部带生成时间与应用/macOS/后端版本。发布检查报告底栏原先一排被截断的文字按钮（发布说明、全部复制、SHA256SUMS）也并进这个菜单，窗口加宽到 640pt。diff 特有的 CSV 保留独立按钮；不带元数据的既有 JSON 导出逐字节不变。
- **发布助手开始记账（发布账本）。** 每次成功运行进账本——版本标签（sheet 和工作区预设新增的可选字段，留空用文件名顶上）、日期、产物路径、SHA-256、结构指纹、卫生计数、各步骤耗时、应用/后端版本。sheet 新增默认折叠的「发布历史」抽屉，列出最近条目、哈希可复制、产物不在时提示「产物已不在」。上限 100 条；随设置备份迁移；失败的运行只留在活动中心历史。
- **发布产物可以和上一次对比。** 发布历史抽屉里每条记录新增对比按钮：账面报告显示日期/体积/文件数变化、结构指纹是否变化（重新打包 vs 内容真变了）、「垃圾回潮」提醒，以及上次 vs 这次的发布卫生清单（可复现 / SHA256SUMS / 签名）——产物不在也能比。两个归档都还在时一键跑完整文件级对比（现有比较视图呈现）。macOS 26 上（Apple Intelligence 可用时）还有一个**AI 总结**按钮，把这份账面对比变成一段总结，点出值得复看的倒退——垃圾回潮、指纹意外变化、校验文件或签名比上次少了——只描述，绝不给「该不该发」的结论。
- **机器可读发布清单。** 「产出与签名」抽屉新增开关，在归档旁写一份 `release-manifest.json`——名字、版本标签、SHA-256、大小与结构指纹，确定性编码，给 CI 和下载脚本用。绝不覆盖（重名自动唯一化）；作为独立计时步骤留痕。
- **一键复制可直接粘贴的发布说明。** 发布检查报告新增「复制发布说明」：带 SHA-256 的下载表、`shasum` 校验命令（写过 SHA256SUMS 时含 `-c SHA256SUMS`）、检查与可复现说明，以及——仅当 GPG 已启用且产物旁有签名 `.szs` 和公钥时——完整的导入与验签指引（与 VERIFY.md 同源文案）。草稿为英文，与其余发布物材料一致。
- **发布前质量门。** 六条规则——可疑条目路径、macOS 垃圾、空目录、未生成 SHA256SUMS、未签名为 .szs、App bundle 签名问题——每条可在新折叠抽屉里设为 关/警告/阻断（默认全关，不开启就什么都不变）。警告显示在检查报告顶部；阻断让这次运行失败并点名触发规则。没检查到的事实（检查关闭时）不会当成问题。
- **可复现构建深度报告。** 右键文件夹 → 可复现性报告：SimpleZip 用可复现 ZIP 设置把它打两次，报告两份归档是否逐字节相同（SHA-256 一致），再如实列出各因素——时间戳已剥离、条目顺序确定，而权限位原样保留、owner/group 与扩展属性 ZIP/7z 本就不携带。两次打包不一致时，把「原样保留」的因素点出来当可疑原因。源只读；两份打包写到临时目录、用后即删。
- **发布目录完整性检查。** 右键文件夹 → 归档工具 → 检查发布目录：SHA256SUMS 覆盖与逐条校验、`.szs` 清单条目逐个对照目录文件、VERIFY.md 引用是否指向已改名/删除的文件、随包公钥能否独立验证随包 `.szs`（临时隔离钥匙环——绝不读写你自己的钥匙环），以及不属于任何发布角色的文件清单。全程只读；走统一报告导出菜单。GPG 关闭时公钥检查如实报「已跳过」。目录里的 `.dmg` 还会被只读挂载一下、确认里面确实有内容（空镜像或挂不上会被标出来）。
- **`simplezip` 命令行大幅增强。** `help <命令>` 给单命令详细用法；`--json` 每命令输出一个机器可读结果对象、`--quiet` 只留错误、`--verbose` 透传引擎原始输出；`simplezip doctor` 体检环境（app bundle、7-Zip 引擎、RAR/GPG 后端、符号链接指向）；命令打错给「你是不是想输」建议；`check` 与多文件 `verify` 末尾给 N 通过/M 失败汇总；`create` 新增 `--level 0-9`、`--exclude-junk`、`--reproducible` 与 `--encrypt`——加密口令从 SIMPLEZIP_PASSWORD 环境变量或终端无回显交互读取，绝不接受命令行参数。还补齐了 shell 补全——`simplezip completions <zsh|bash|fish>` 打印子命令与旗标的补全脚本——以及一份 man page（`docs/simplezip.1`，`man -l` 可看）。
- **五个新的快捷指令动作。** 验证校验文件（逐条核验 SHA256SUMS 类文件，不安全路径直接拒绝，返回真/假）、比较归档（条目级比较，返回是否一致）、创建发布包（无头跑发布助手：打包 + 检查 + SHA256SUMS，可从列表里挑选工作区预设、或用变量按名字匹配；`.szs` 签名只在交互式流程里做，不会无人值守）、搜索归档内容（返回路径包含你输入文本的条目——条目名需要密码的归档不会被列出）、检查归档（不解压就看文件数、总大小、macOS 垃圾与可疑条目路径，无标记时返回真）。归档名沿用统一校验；每次运行进活动中心并标记为快捷指令来源，发布运行照常进发布账本。
- **设置「自动化」页。** 一页看全所有外部驱动方式：命令行工具（安装区从「通用」搬来——单一归属）、八个快捷指令动作与「打开快捷指令」按钮、Spotlight 索引开关（发布包、最近任务与打开过的归档内容，默认开）、simplezip:// 命令（一律 app 内确认）、Finder 服务总览、按来源的活动中心使用统计，以及新安全开关「允许自动化使用预设密码」（默认开；关掉后无人值守的快捷指令/CLI 只试空密码）。该开关随设置备份迁移。macOS 26 上,快捷指令一节还多了个**建议一个自动化**按钮(Apple Intelligence 可用时):本机 AI 看你实际怎么用这个 app——最近的操作类型与触发来源计数(只看聚合数字,绝不看文件名或内容)——起草一两个贴合你习惯的 Shortcuts 点子 + 详细设置步骤,落进可编辑文本框。这是给你自己照着搭的草稿,不会创建、安装或运行任何内容。
- **任务记录来源。** 任务会记录发起表面——应用内、命令行、快捷指令/Siri、`simplezip://` URL 或 Finder 服务——并随任务历史持久化（来源筛选与自动化统计的地基；旧历史照常加载，老记录按应用内计）。
- **开发者工具：格式兼容性实验室。** 选一个小文件夹，实验室对真实后端把它在 zip、7z、tar、tar.gz 之间往返——验证哪些属性得以保留：结构、可执行位、扩展属性、符号链接、时间戳、注释往返（zip）与可复现性（zip/7z）。探针加在临时副本上，原文件夹绝不被改动。按设计藏在开发者工具里。AI 数据区也会显示动态文件夹的隐藏竞争池、已通过复核和复核中数量,所以「0 可见」时不再看不出生成器到底有没有在跑。
- **发布包与归档任务现在都是 App Intents 实体。** 发布账本里每条记录暴露成可查询的「发布包」实体（版本、日期、格式、SHA-256、文件数、可复现与校验文件标记），活动中心历史里每个任务暴露成「归档任务」实体（操作、来源、状态、开始时间）——这样自动化可以引用某个具体的历史发布、或「那个失败的解压任务」，而不是一条裸文件路径。两者都只读各自的存储（任务侧新加了一个 nonisolated 只读历史查询面），绝不碰归档内容，也不做任何写入或安全判定。这是下面 Spotlight 索引与实体感知 intent 的地基。
- **AI 起草你的发布文书（macOS 26）。** 三个端上 AI 生成器接进发布相关界面，每个都把结果落进可编辑文本框供你审阅后自取：**起草 VERIFY.md** 把发布目录检查结果变成可直接粘贴的校验说明——按目录里实际有的东西给出真正的 `shasum`/`gpg` 命令——外加一节「缺失 / 建议」；**起草发布正文** 把发布检查报告变成一段 GitHub Release 公告（下载、SHA-256 与校验步骤；只有当产物旁确实有签名 `.szs` 和公钥时才出现签名小节）；「复制为 Issue」旁的 **智能归类** 按钮则为诊断 issue 建议标题、标签和一段总结。全部只读、只吃报告里的具体事实（不会编造文件或版本号）、端上推理，仅在 AI 助手可用时出现。

### 交互与界面 (UX)

**面向用户**

- **发布说明与预设命名改用 SwiftUI 文本框（吃 Writing Tools）。** 「复制发布说明」现在打开一个可编辑文本框，而不是直接复制到剪贴板（框里仍有复制按钮）；保存或重命名发布助手工作区预设也改用原生 SwiftUI 输入框，不再是普通 alert——这样在 macOS 15.1+ 上你可以选中文本、用 Writing Tools（改写、校对、缩写）后再复制或保存。
- **文件关联拆分分组 + 每组一键全设。** 设置 → 文件关联(及欢迎助手)把「压缩包」与「磁盘镜像与安装包」(dmg/iso 挂载、pkg 安装)分成两个组,每组加「全部设为默认」一键关联整组。一键全设只按**实际生效数**报结果——.dmg/.pkg 这类会弹系统确认框,用户拒绝就不计入,不再误报「已设 N 个」。
- **折叠行点整行就能展开。** 设置的「视图」「GPG 高级」折叠组、以及更新日志的每个版本行,现在点 header 任意位置都能展开/收起(不必只点小三角),与活动中心同款手感。
- **活动中心整体换上层叠渐变 chrome。** 侧栏选中行、每个分页头部（发光图标瓦片 + 一行副标题）、任务卡（按分类着色——归档蓝、文件操作橙、撤销重做紫）、写入锁与等待组组头、空状态，全部换上帮助页/欢迎助手同款 chrome：tint 渐变底、顶部高光、渐变描边、彩色阴影。刻意做了性能阉割：任务卡运行时每秒重绘多次，chrome 全部纯静态——无 hover 特效、无新增动画；只保留运行中任务原有的旋转描边。
- **活动中心图标精修。** 内容区彩色瓦片改纯色（去渐变）并大幅变浅，卡片容器底色远浅于帮助页原版，侧栏保持原样；任务行状态图标和逐文件图标从淡色圆底换成白符号圆角矩形彩色瓦片，与全应用瓦片家族一致。
- **活动中心卡片有了呼吸感。** 页面左右留白、卡片间距与内边距全部加大，命令输出框从 142pt 提到 300pt。展开详情（哈希结果、传输日志、比较树）封顶 320pt、超出在框内滚动——给几百个文件算哈希不会再把一张卡撑满三屏。
- **任务卡控制图标全部上色。** 暂停/继续、取消、重跑、从失败步继续，以及展开详情里的复制/导出按钮全部换成彩色圆底图标（复制蓝、导出青、命令紫、暂停橙、取消红），不再是裸灰描线。
- **任务卡帮助页同款展开手感。** 点击卡头任意位置即可展开/收起详情（行内按钮仍各管各的）；详情开关换成帮助页风格的彩色圆底 chevron，展开时旋转。
- **活动中心头部运行时不再被挤压。** 有任务运行时暂停/睡眠/取消控件改纯图标、页标题钉住，不再塌成竖排；状态菜单旁那个装饰性筛选小图标删掉了。
- **失败红点随滚动逐个消除。** 每张失败卡片在真正滚进屏幕看得见的一瞬间就消掉自己的红点：看到一个消一个，全部滚完红点全消。还在屏幕外的失败照常计数，已看状态重启后保留。
- **导出报告控件改成原生下拉菜单。** confirmationDialog 在 macOS 上塞不下「4 个导出选项 + 取消」（加取消会把导出 JSON 挤掉），改成与「套用模板」同款的下拉：全部选项常显，点菜单外 / Esc 即取消（天然的反悔出口）。
- **只读归档保留分析菜单。** 打开只读格式（tar、rar、zst、嵌套归档、`.siz` 的内层归档）时，空白处右键不再丢掉「元数据报告」和「复制打开性能报告」——它们什么都不写，理应也在这里。
- **报告之间可以跳转。** 发布检查报告新增「空间分析」按钮，对同一归档直接跑分析。
- **归档菜单补齐文件夹级发布 / 分析工具。** 批量体检、数据救援、快速核对发布组、发布目录完整性检查、可复现性检查——原先只在文件浏览右键里——现在也进了「归档」菜单，紧挨发布包检查与空间分析。每个工具自己校验选区，选错了只会提示你先选一个归档或文件夹。
- **解压对话框把行为开关收进两个抽屉。** 冲突自动重命名、跳过 macOS 垃圾、跳过符号链接收进「内容处理」抽屉，完成后在 Finder 显示、原包移到废纸篓收进「解压完成后」抽屉；对话框打开时只剩目标目录、密码、预检概要和决定文件落点的开关。行为不变；预设依旧一键设置全部开关。
- **转换对话框把六个行为开关收进一个抽屉。** 保留结构、跳过相同、失败重试、转换后校验、原包进废纸篓、套用格式默认值收进「转换行为」抽屉；打开时只剩目标格式、密码、输出目录、保真度报告和源列表。行为不变。
- **签名清单（.szs）对话框把可选项收进抽屉。** 标题/说明收进「元数据（可选）」抽屉，加密整块（开关、收件人、口令）收进「加密为 .gpg」抽屉（已开加密时初始展开）；打开时只剩根目录、文件、签名密钥、验证材料和输出位置。行为不变。
- **发布助手把伴生产物开关收进抽屉。** SHA256SUMS、签名为 .szs 收进「产出与签名」抽屉；定义这次发布的三个打包开关（排除垃圾、可复现、发布检查）留一级。行为不变。
- **设置：视图页把常用选项放前面。** 占地方的列开关和实时预览挪到最底部；显示、分组、展开与记忆、显示名排在上面。
- **设置：呈现类选项从「浏览器」搬到「视图」。** 隐藏文件组折叠策略、分组时隐藏文件去向、「展开与记忆」整组、隐藏后缀名显示管理（新「显示名」区）现在都在视图页；浏览器页只留决定**列出什么**的选项（显示隐藏文件与判定方式、符号链接、跟随 Finder 结构）。存储 key 一个没改，设置不会重置。
- **发布包检测报告窗口加高。** 报告滚动区上限调到 580pt，常见显示器上一份典型检测报告不用滚动就能看完；内容少的报告仍按实际高度收缩。

### 修复 (bugfix)

**面向用户**

- **残留的临时暂存目录在启动时清扫，不再只靠正常退出。** 归档内原位编辑和 `.szs` 创建的临时目录靠 `defer` 删除——崩溃或强制退出就会残留（里面是明文归档内容）。现在启动时也会在后台清扫本次会话之前所有陈旧的 `SimpleZip*` 临时目录（当前会话在用的暂存不碰）。
- **修改密钥 passphrase 不再把旧口令暴露给 `ps`。** 以前改密码流程把**旧** passphrase 当作 `gpg` 命令行参数传（`ps` / 活动监视器可见），只有新口令走 stdin。现在新旧口令都通过 `--pinentry-mode loopback --passwd` 走 stdin，符合 app「密钥只走 stdin」的惯例。（经 GnuPG 2.5.x 验证。）
- **「恢复默认」在钥匙串密码没清掉时不再谎报成功。** 如果从钥匙串清除预设密码失败（钥匙串锁定 / 权限），以前照样提示「已恢复」——现在会说明偏好已恢复、但密码仍在钥匙串里、需手动删除。
- **任务列表可以顺畅滚到底了。** 懒加载列表边滚边重估卡片高度——滚动条抽搐、底部卡片滚不进来。列表改为一次性布局（卡片数量受历史上限约束），滚动条随之稳定。
- **展开的任务卡滚远后不再自动收起。** 任务列表会回收滚出屏幕的行，行内展开标记跟着被清掉——现在展开状态按任务 id 记在列表层，回收重建也不丢。
- **元数据报告条目数不再打印空的 `()`。** 它误用了带「总大小」占位符的文案，而元数据聚合不带这个字段。

**面向开发者**

- **SHA-256 失败时发布不再谎称写了 SHA256SUMS。** 哈希步故意吞掉错误好让运行仍能出报告——但发布账本和生成的发布说明按「请求里勾了写校验文件」的意图记成「已写校验文件」，而非实际结果。现在检查报告带上真实结果（只有 SHA256SUMS 文件真写出来才为 true），账本与发布说明都读它。

### 改进

**面向用户**

- **浏览 /Applications（及其他图标密集文件夹）不再卡顿。** 以前每个可见行每次重绘都在主线程取真实文件图标并解码 ICNS——对一套标准 /Applications 一轮约需 740ms。现在图标从预栅格化位图缓存里取（绘制约 1ms）；首次见到的行先显示类型级占位图标，真图标在后台取回栅格化后原位换上。表格 cell 改为真正复用，不再每个滚动帧拆掉重建文本框/图片视图/约束；应用显示名也不再每次刷新文件夹重读 Info.plist。
- **文件哈希更快。** CRC32 列改用 slice-by-8——每步吃 8 字节、8 张表并行查，不再逐字节计算——大文件上约 2-4×；纯可移植 Swift，Apple Silicon 与 Intel 走同一份代码，输出与之前字节级一致（已用标准向量与随机流式对拍验证）。多文件哈希现在跨所有性能核并行，不再单线程，对大文件夹生成 SHA256SUMS 快得多；并发上限取核心数，绝不耗尽文件句柄，结果顺序不变。
- **发布助手交互打磨。** 归档已打出但后续步骤失败的运行，任务卡上直接给「从失败步继续」——检查/校验/清单对既有产物重跑，不重新打包；检查报告新增「步骤」小节展示各步耗时；工作区预设支持重命名；运行中任务显示产物完整输出路径。
- **打开大归档更跟手，空间分析瞬开。** 归档列表一出来，路径安全扫描和空间分析就都在后台先跑好，不再等你点了才算——所以对一个超大归档，点「空间分析」不会在那一下于主线程现算 10 万条（以前会让窗口短暂卡住），而是直接拿后台预热好的结果。预热跟当前这次列表绑定，导航走开就作废。

**面向开发者**

- **发布流水线抽成可复用引擎并支持续跑。** 打包、发布检查、写校验文件作为命名步骤计时留痕，随历史持久化；引擎被快捷指令的「创建发布包」与 GUI 共用，打包之后失败的运行可拿既有产物重新检查而不必重新打包（重新打包永远生成新唯一文件名，绝不覆盖）。
- **通用诊断报告带上更多现场。** 最近任务现在记录来源（应用内 / 命令行 / 快捷指令 / URL / Finder）并按来源汇总；报告还附上一份已脱敏的设置快照（与设置备份白名单同源——绝不含口令 / 私钥），方便维护者复现用户配置。全部仍走密码脱敏。
- **活动中心 chrome 收进一个共享组件。** 不再是帮助页 / 欢迎助手 / 关于页三份手拷贝。
- **纯值类型下沉到可测的 Core。** 原本在 Features 里、无副作用的 Codable 模型——发布检查报告、传输/冲突模型、哈希模型——搬进 SwiftPM 可测的 Core target,补测试钉死契约(Codable 往返、未知枚举值降级、统计合并、汇总格式)。行为不变。
- **热点路径加 os_signpost 打点。** 共享的 PerfSignpost 把后端子进程生命周期(每次 7zz/gpg/rar 运行)、列归档、安全/空间/发布检查扫描、比较、文件图标解码都括成 Points of Interest 区间,在 Instruments 中可直接定位。零行为变更、零用户可见,纯性能剖析管线。

### 杂项

- 发布流水线步骤名去掉多余的「步骤:」前缀（报告里已有「步骤」小节标题）。
...
- **界面：欢迎助手换上帮助页同款的叠层华丽外观。** 每个步骤现在坐在步骤色渐变外壳里——顶部高光、渐变描边、发光图标瓦片、彩色阴影——控件保持在实底内卡上，内外层次清晰；第一页加上帮助页风格的 hero 瓦片，步骤圆点会发光，完成徽章升级渐变庆祝。动画保持克制——悬停只动阴影，不旋转、不缩放容器。
...
- **UI：创建对话框可一键转入发布助手。** 「套用模板」旁新增「使用发布助手」按钮：关掉创建对话框、带着已填内容进入发布助手——文件名、保存位置、ZIP/7z 格式照搬；选区恰好是单个文件夹时直接作为产物目录，其他选区源目录留给你挑。
...
- **新增：复制为 GitHub Issue。** 任务详情面板新增按钮，把同一份诊断排成可直接粘贴的 issue Markdown：环境表（app/macOS/后端版本，启用时含 GPG）、「发生了什么 / 复现步骤」占位、出错操作与脱敏后的错误信息、折叠块里的脱敏日志尾部、文件系统现场。密码不会进入——复用诊断报告同一套脱敏。
...
- **修复：文件夹导航不再闪一帧未分组视图。** 把列举挪下主线程（启动冻结修复）把一次视觉事务拆成了两半：表格先用「新文件夹的分组配置 + 旧 items」重建了一帧，每次导航都先闪未分组再跳成分组。带时序探针经确认（两次 reload，第一次内容是旧的）后，文件表现在跳过那个中间帧、只渲染一次——items 与分组在同一事务落地，和当年同步版一样。
- **新增：完成后睡眠。** 任务运行期间，活动中心标题栏多一个「完成后睡眠」开关：最后一个运行中的任务结束的那一刻让 Mac 进入睡眠——给通宵大解压用。仅本次会话有效、用后自动复位，下一批任务不会意外把机器休眠；睡眠走标准 System Events 通道（macOS 首次会弹一次自动化授权）。
- **新增：发布助手——打包、检查、校验、可选签名，一条流。** 工具 → 发布助手 选一个产物目录，整套发布流程跑成一个可取消的任务：打包为 ZIP 或 7z（macOS 垃圾 `.DS_Store`/AppleDouble/`Thumbs.db` 不进归档，可复现压缩默认开启——同一份输入永远同一个 SHA-256）、对产物运行完整发布检查（完整性测试、可疑路径、垃圾、空目录）、在归档旁写出 GNU 兼容的 `SHA256SUMS`，开启 GPG 时还能直接接进现有的签名清单（`.szs`）对话框、新归档已预选。输出名不会覆盖已存在文件，每一步的结果都落进活动中心详情，跑完自动弹出检查报告。
- **新增：真·命令行伴侣——终端里的 `simplezip`。** 设置 → 通用 → 命令行工具 一键在 `/usr/local/bin/simplezip` 安装符号链接——目录不可写时弹标准的 macOS 管理员授权窗口（可复制的 `sudo` 命令仍作兜底），链接存在时始终提供卸载按钮。五个子命令跑的是和 app 同一套内置引擎，有真实 stdout 和退出码（0 成功 / 1 有失败或差异 / 2 用法错误），可进脚本和 CI：`check` 测试归档完整性，`compare` 逐条目比较两个归档，`create` 创建归档（格式按输出扩展名推断、自动套用该格式已保存的默认值、不会覆盖已存在文件），`verify` 校验 SHA256SUMS/校验文件（与 app 内同一套不安全路径防护），`open` 把文件交给 app 打开。每条完成的命令同时记录进活动中心——app 在跑时经跨进程通知实时出现，没在跑则合并写入持久化历史，命令输出可在任务详情里查看。`create --template <name>` 按稳定 slug 套用内置任务模板（github-release、windows-friendly、max-7z、encrypted-delivery、source-code、backup——与创建对话框「套用模板」同一目录）；加密归档现在也能用：CLI 弹出小型密码窗口（不进命令行参数、不在终端回显），最多三次，取消即跳过。CLI 输出仍仅英文（进程脱离 app bundle 运行时本地化解析不到——经实际验证确认）。
...
- **新增：统一密码中心——同一个包，只问一次。** 完整性测试（单包与批量）获得密码智能：遇到加密包先**静默**依次试 会话密码缓存（本包验证过的口令 → 本会话最近一次成功的口令）和预设密码，全都不行才弹密码框——等待输入期间，任务详情日志和状态栏会显示「等待输入密码」，不再像卡死。验证成功的口令记入会话缓存，同口令的一批包只问第一个、其余静默通过；在密码框点取消＝跳过该包（记为「已跳过」、可重试），不会被打成失败——单包测试时取消则整个任务按「已取消」处理。Finder 自动解压接入同一套大脑：预设密码之后会拿会话缓存的口令静默重试再放弃。口令一律走 PTY 应答通道喂给 7zz，不会出现在可见命令行参数里；GPG 私钥口令仍然只归 pinentry 管。
- **新增：Zstandard 与只读格式家族——zst、tar.zst、ISO、CAB、CPIO、XAR、PKG 现在都能用 SimpleZip 打开。** 全部经内置 7-Zip 26.01 验证：`.zst`/`.tzst` 像普通归档一样列出、解压、测试（`.tar.zst` 走与 `.tar.gz` 相同的双层管道）；ISO/UDF 镜像、CAB、CPIO、XAR 容器与安装器 PKG 均可浏览和解压。这批格式诚实地保持只读——7zz 没有 zstd 编码器（`a -tzstd` 返回 E_NOTIMPL），其余格式它也写不了——所以写能力门控会给它们挂「只读」徽章并提示可转换为 ZIP/7z，而不是假装能写。文档类型以 Alternate 等级声明，「打开方式」可用且不抢系统默认。格式能力表补上全家族行（Zstandard、ISO/UDF、CAB、CPIO、XAR/PKG——解压/测试为是,其余诚实为否,附本地化后端说明），文件关联新增 Zstandard 条目（`.zst`/`.tzst`）。后续又补充两项：zstd 的 `-slt` 列表不带条目路径（gzip 带），导致 `.zst`/`.tar.zst` 打开是空列表——解析器现在按 7zz 解压产物的命名规则合成内层名；压缩 tar 壳（`.tar.gz`/`.tgz`/`.tar.zst`/`.tzst`/`.tar.bz2`/`.tar.xz`）打开时**直接进入内层 tar**——地址栏显示 `…/foo.tar.zst/foo.tar` 虚拟链，「上一级」直接回真实文件夹，导航全程不漏 `/tmp`。
...
- **新增：「macOS 元数据」帮助页讲清进包后还剩什么。** 设置 → 帮助（与活动中心帮助页）新增主题，内容全部来自内置引擎的实际测试：哪些格式保留 Unix 权限（全都保留）、哪些保留扩展属性如 Finder 标签 / 隔离标记 / resource fork（tar/tar.gz 保留,ZIP/7z 不保留）、归档实用工具的 __MACOSX 条目是什么、清理垃圾为何删它们、以及任何格式都不承诺的东西（创建时间、访问时间、ACL）。结尾给出经验法则：发 Mac 用 tar.gz,发全世界用 ZIP + 清理垃圾。
- **新增：可复现压缩（ZIP 与 7z）。** 创建对话框高级抽屉里的「可复现压缩」开关不再存修改时间——同一输入每次打包都得到字节级一致的归档（同一个 SHA-256，端到端验证；条目顺序本就稳定）。适合 GitHub Release 与可验证构建，和「导出 SHA256SUMS」正好配套。解压时条目时间显示为 1980-01-01。tar 系坦白排除：它走系统 tar，没有时间戳钳制参数。
...
- **新增：校验文件支持（SHA256SUMS 一族）。** 右键选中文件 → **生成 SHA256SUMS**：写出 GNU 兼容校验文件（`sha256sum -c` 直接可验；不会覆盖已有文件）。右键校验文件（SHA256SUMS、checksums.txt、`.sha256`/`.md5`/`.sfv`）→ **验证校验文件**：对每个列出的目标重算摘要，逐文件通过 / 失败记录在活动中心——GNU、BSD（`SHA256 (file) = …`）、SFV 与裸摘要格式全部支持；校验文件里的不安全路径只跳过、绝不跟随。发布包检查新增**导出 SHA256SUMS** 按钮，没装 SimpleZip 的人也能验证你的发布产物。
...
- **新增：统一写入闸门守住每一次归档改写。** 所有安全写回操作（拖入/新增条目、重命名、批量重命名、删除条目、清理元数据垃圾、注释编辑）现在都经过同一把「按归档互斥」的写锁：同一个包的写任务排队执行而不是互相竞争——以前两个任务同时改写一个 zip 会各自基于旧包生成新包，后落地的把先落地的成果整个覆盖掉。排队期间任务详情日志和状态栏会显示「正在等待另一个任务写完此归档」。在此之上，每次写入前还会核对归档仍是你打开时的那个文件（列出时记录 size/mtime/inode 快照戳）：期间被 Finder、其他 App 或其他窗口改过——包括改写准备过程中被改——写入会停止并提示「已被外部修改」，不会覆盖外部改动；刷新后重试即可。任何写回成功后，打开同一归档的其他窗口 / 标签页自动刷新。

## 0.4.2

- **修复：大目录不再把文件浏览卡死。** 「包」判定（.app/.bundle 还是普通文件夹）以前是现场解析符号链接 + 问 LaunchServices——而且发生在排序比较器里（万项目录就是几十万次调用）、表格每一行、以及拖拽的每次鼠标移动；列目录任务卡死在路径解析里直到 app 被强杀。现在列目录时每项判定一次（用批量取的 isPackage 资源值，只有符号链接才解析目标）存进条目，下游全部零 IO 读字段。
...
- **按反馈再扫一轮。** ① 工具栏动态双操作更聪明了：不会与常驻按钮重复、按选中内容细分（分卷→合并、两个归档→批量测试+比较、`.szs`→快照比较、普通文件→加密签名或重命名/副本/拆分…）、作为独立玻璃囊紧贴常驻簇、剪贴板里真有东西才出现「粘贴」。② 新增顶级**操作**菜单——解压/测试/哈希/比较/转换/拆分/合并/发布检查/注释/垃圾清理/安全报告全部入栏（GPG 项按总开关门控）。③ 虚拟浏览的「保存解密文件」有了专门绘制的对话框。④ 关于页的 SimpleZip 字标改为液态玻璃,玻璃色跟随系统强调色。⑤ 欢迎助手重绘：真 app 图标 hero、分段步骤点、主题色柔光、内容放得下就没有滚动条,完成页新增版本亮点卡并指路 设置 → 帮助。
...
- **UI：创建前预检升为一级。** 预检从折叠抽屉搬出来，变成基本选项正下方的常驻概要条：对话框打开即自动统计，排除规则 / 分卷大小 / 格式一变就实时重算——文件数、总大小、排除 / 符号链接 / 包数量、分卷估算和输出重名警告一目了然。
- **新增：「不解压 macOS 元数据垃圾」。** 两个解压对话框都有了这个开关：合并前在 staging 副本上清掉 `.DS_Store`、`__MACOSX`、AppleDouble `._*`、`Thumbs.db`、`desktop.ini`——目标目录原有文件不会被触碰。实现备注：清扫用 POSIX `readdir` 走树,因为 Foundation 的目录枚举会把 AppleDouble 文件偷偷藏掉。
...
- **安全（本版自审）：两处加固。** ① 新增的等价命令日志现在会把 `--passphrase` 后面的值打码——GPG 改口令流程出于可靠性折中把旧口令放在 argv 里，不打码的话它会被持久写进活动中心历史和诊断报告。② 条目名是不可信输入：归档内的 增 / 删 / 改名 现在把所有条目名放在 `--`（参数结束标记）之后，与 0.4.1 解压侧的加固同口径——恶意构造的包里名叫 `-r`、`-p…` 的条目在编辑该包时不再可能被当成 7-Zip 开关解析。经既有条目编辑测试套件验证。
- **新增：创建对话框的智能卡签名状态。** 选中的签名密钥在智能卡 / OpenPGP 令牌上时，GPG 区会提前讲明白——签名时必须插卡、系统可能弹 PIN——并检测卡是否在位：绿色「已检测到卡」或橙色「未检测到卡——请先插卡再创建」，外加**重新检测**按钮。不会再因为没插卡而莫名签名失败。
...
- **新增：复制等价后端命令。** 每次后端调用都会在任务输出日志开头打出可直接粘进 shell 的等价命令行（`$ …`），活动中心任务详情新增终端按钮一键复制本次运行的全部命令。无需打码——口令在设计上就从不进命令行（走安全提示通道）；提示里注明路径可能指向本次操作的临时工作目录。既方便 debug，也让你看清 SimpleZip 到底执行了什么。
...
- **新增：归档内批量重命名。** 在可编辑的 ZIP/7z 里选中多个文件 → 右键 → **批量重命名…**：替换文本、加前缀 / 后缀（插在扩展名前）、转大小写、按序号编号——每条旧名 → 新名实时预览。冲突（互撞或撞上未改名的条目）和非法名标红并被跳过。整批在工作副本上跑一次 7-Zip 重命名、成功后原子替换原包——失败不会留下改了一半的包。
...
- **新增：解压对话框的解压前预检。** 解压对话框现在先给一份安装器式概要：将解出多少文件 / 文件夹、总大小，以及针对性提醒——含加密条目（需要密码）、目标位置已有同名项（会进入冲突处理）、分卷集缺卷、可疑路径条目、符号链接。换目标目录时覆盖检查实时重算；读不了的包（如加密 header）只是收起概要，不会阻止解压。
...
- **新增:编辑压缩包内的文件并写回。** 用外部 app 打开归档内某文件,改完回到 SimpleZip,若检测到文件被编辑,会询问是否把改动写回压缩包 —— 同样安全写回(复制→更新→原子替换),失败不会破坏原包。
- **新增:在 ZIP/7z 里右键即可编辑 —— 添加文件…、粘贴、重命名、删除条目。** 浏览可编辑归档时,右键菜单多出这些操作(**白名单**:只对 zip/7z 出现,TAR/RAR 等只读格式、嵌套/临时归档都不显示)。每个都走活动中心 + 安全写回(复制→更新副本→原子替换),失败不会破坏原包。删除会二次确认。
- **新增:把文件 / 文件夹拖进打开的 ZIP/7z 即可加入。** 拖文件到打开的 zip/7z 会在你当前浏览的归档内文件夹增/替换条目,走活动中心,用**安全写回**(复制原包→在副本上更新→原子替换,失败不会破坏原包)。只有**真实顶层**压缩包接受拖入(嵌套 / 临时包、其它格式都不接受)。
...
- **新增:设置 → 压缩里的「格式能力」对照表**,直观展示各容器(ZIP / 7z / RAR / TAR / TAR.GZ / DMG / .gpg / .siz / .szs)各自支持哪些操作 —— 创建、解压、加密、头部加密、分卷、测试、注释,RAR 创建标注需自行安装 rar 工具。
...
- **修复:智能卡上的密钥在拔卡后不再显示「在卡上」(普通本机私钥也可能被误判)。** 「在卡上 / 已 stripped / 本机」状态来自 `gpg --list-secret-keys --with-colons` 的第 15 字段(`+`=本机完整、`#`=stripped、serial=在卡上),但解析读的是第 14 字段(对 `sec`/`ssb` 恒空),于是自己从来判不出卡上状态——只在插卡时靠第二条路径(`gpg --card-status`)兜回来。一拔卡,这把钥匙就错误退化成普通本机私钥。现在解析读对字段(并把 `+`/`#`/serial 区分开,普通钥匙不会被误标为卡上),并加了解析回归测试。
...
- **改进:从压缩包拖出条目现在会进活动中心。** 拖出解压以前是静默跑的(没任务、没进度、不能取消)。现在和其它解压一样是托管任务——在活动中心有进度条、解压途中可取消(不强行弹窗,快速拖动时保持安静)。
...
- **修复:打开被 `.gpg` 包住的 `.siz`(`name.siz.gpg`)时,地址栏会泄漏出 `/var/folders/…` 的 scratch 路径。** 之前解密 `name.siz.gpg` 会把内层 `.siz` 解到加密临时卷、再当成一个散文件显示;打开它时签名流程锚定的是那个 scratch 路径。现在解密后**直接把内层 `.siz` 送进 unwrap + 验签流程**(篡改照样会被验签拦住),地址栏锚定**原始 `.gpg`**——不会显示临时路径,「上一级」回到 `.gpg` 所在的真实文件夹。
...
- **双击 GPG 加密文件（`.gpg` / `.pgp`）即可打开，就像打开带密码的压缩包。** SimpleZip 先解密（passphrase 由 gpg-agent / pinentry 弹框输入——SimpleZip 从不经手你的 GPG 私钥 passphrase；授权期间状态栏显示「正在解密…」），再把产物**留在 SimpleZip 内打开、不会直接丢给 Finder**：是压缩包就原地浏览，否则在 SimpleZip 自己的浏览器里展示——两种情况地址栏都显示原 `.gpg` 名而非临时路径。关键在于：`.gpg`/`.pgp`/`.asc` **不会仅凭后缀就当成加密数据** —— SimpleZip 先读 OpenPGP 包头嗅探（装甲行 / 首包 tag，绝不真去解密、不弹密码、取消时也不会再误报「不是加密数据」）：公钥/私钥导出会被导向「导入钥匙串」对话框而不是去解密，分离签名则不碰。从 Finder 打开时复用当前窗口而不是新开标签。需在「设置 ▸ GPG」中启用 GPG 集成。
- **修复：打开嵌套压缩包（压缩包里套压缩包——`.tgz` 里的 `.tar`、zip 套 zip 等）会漏出 `/var/folders/…` 临时路径、且「上一级」错乱。** 之前嵌套档案项被交给系统、从临时目录把解出来的副本无上下文地重新打开，于是地址栏和「上一级」暴露 `/var/folders/…`。现在嵌套档案**在 SimpleZip 内原地打开**，地址栏把整条嵌套链按虚拟目录堆叠出来，任意层数都能看懂自己在哪（如 `…/Desktop/xx.zip/xa/a.zip/b.zip/c.zip`）；从嵌套档案「上一级」回到最外层档案所在的真实文件夹，不会显示临时路径。
- **解密 / 解压的临时文件现在放进「每次会话随机密钥的加密临时卷」，关闭压缩包即时清理。** 之前打开 `.gpg`/`.siz`、浏览压缩包、或打开档案内文件时解出来的明文，会一直裸躺在 `/var/folders/…` 直到下次启动——这等于把加密的意义破坏了。现在首次需要时 SimpleZip 会挂一个 **AES-256 加密磁盘镜像，密码是只存在于本次会话内存里的随机串**；所有临时产物都放进去，退出即 detach + 删镜像，崩溃残留的镜像下次启动清扫（磁盘上是密文，没有内存里的密钥谁都打不开）。解密 `.gpg`/`.siz` 是 **fail-closed**：明文只准写进加密卷（卷挂不上就报错、不会退回明文落盘）；会话中途卷被卸载也会被检测到并在解密前透明重挂。离开压缩包（进文件夹 / 开别的档案）会**立刻清掉该档案的临时**，不等退出 app。
...
- **修复：冷启动打开 `.zip`/`.7z`（开了自动解压）会错误地拉起主窗口。** 之前那个冷启动补窗修复对**任何** Finder 打开都建主窗口；但开了自动解压时,解压必须留在独立浮窗里、不会拉主窗口。现在冷启动遇到自动解压压缩包直接走浮窗。
- **自动解压彻底和主窗口脱钩 —— 包括 `.siz` 与 `.szs`。** 开了自动解压后，从 Finder 打开**任何**受支持文件（`.zip`/`.7z`/… **以及** `.siz`/`.szs`）都完全留在独立浮窗里：验签、解密、解压、`.szs` 清单校验全在浮窗内完成，主窗口不会被创建、拉起或隐藏（冷启动、热运行都一样）。每个浮窗都加了 **「在主窗口打开」** 按钮，需要时（比如浏览 `.siz` 内容、把 `.szs` 以虚拟目录浏览）才由你主动拉起主窗口。关了自动解压则行为不变（仍在主窗口浏览）。
...
- **主窗口支持系统原生标签页（⌘T）和多窗口（⌥⌘N）。** 按 ⌘T 在当前窗口里开新标签、⌥⌘N 开一个独立的新窗口、视图 →「显示标签页栏」（⇧⌘T）可常显标签栏。SimpleZip 已在运行时从 Finder 打开压缩包或文件夹，会在新标签里打开，而不是替换掉你正在看的内容。这是真正的 macOS 窗口标签 —— 只有一个标签时标签栏自动隐藏，「移动标签到新窗口」/「合并所有窗口」都能用。每个标签 / 窗口完全独立（各自的文件夹、选区、撤销历史）。Dock 图标右键菜单也提供「新建标签页」和「新建窗口」，和「文件」菜单保持一致。设置、关于、更新窗口仍独立，不会并进标签组。
...
- **修复：智能卡 / stripped 密钥不再显示「导出私钥」。** 私钥在智能卡上（或已从本机 stripped）的密钥，本机根本没有可导出的私钥材料 —— `gpg --export-secret-keys` 只会导出一个无用 stub，给人「我备份了私钥」的错觉。现在只有本机真正持有可导出私钥材料时才显示该菜单项。（顺带说明：「导出公钥」用的是 `gpg --export`，只导公钥、绝不含私钥 —— 经验证确认。）
...
- **列表列设置从 Settings 搬到顶层「视图」菜单**
  ...
  - ColumnsPane 网格：4 个共用列（大小 / 种类 / 修改时间 / 创建日期）现在两侧同一行对齐，左右对照可直观匹配，不必反复在两列之间找。
...
- **`.szs` 签名清单格式 v1 端到端实现**（设计 + Core + GPG + UI + UTI）
  - **格式**：`.szs` = GPG clearsigned 签名的 JSON 清单。**一份签名清单 + 多个外部文件留在原地** —— 跟 `.siz`（单文件容器）互补。用例：发布物（app + LICENSE + README + checksums.txt）、镜像树、按文件级完整性验证。`cat foo.szs` 直接看 JSON、`gpg --verify foo.szs` 命令行能验，单文件即可完成，不需要 sidecar `.sig`。
...
- **GPG 钥匙管理 Round 2：子密钥展示 / 公钥隔离 / 默认签名密钥 / 智能卡反查 / 右键菜单**
  ...
  - **高级区新增**：「SimpleZip 私有 ring」路径展示，让用户直观查看隔离的 keyring 文件位置（方便手动 `gpg --no-default-keyring --keyring <path> --list-keys` 验证）。
  - **后端**：
...
- **新功能：GPG 集成（A 阶段基础 + 钥匙串 + .siz 容器签名）**
  ...
  - 主开关 `gpgEnabled` 默认 false —— 关掉时其它入口（创建对话框的「GPG 签名」复选框、未来的验签徽章等）全部隐藏，对不使用 GPG 的用户保持隐藏；设置 pane 始终可见让用户能开它。
...
- **新功能：「个人收藏」镜像 Finder 侧栏**
  ...
  - **架构选择**：`finderFavorites` 放在 `ArchiveBrowserModel` 作为 `@Published`，不放在 Sidebar 的 `@State`。前者通过 ObservableObject publisher 传给 `@ObservedObject`，更新可靠；后者在 NavigationSplitView 的 sidebar 列里有路径会让 @State 重置（经验证，主线程赋值后下一帧 getter 仍读初始值）。
