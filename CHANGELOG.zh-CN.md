...
- **运行状态能检测并修复后台 AI 服务。** 开启静默后台索引后,设置 → 运行状态会显示后台 AI 服务是否已注册并响应,陈旧时提供一键修复 —— 首次引入该功能、应用更新后、或更换签名身份后都可能陈旧。如果你在 系统设置 → 通用 → 登录项 把该服务关掉了,SimpleZip 会尊重你的选择:绝不偷偷重新打开,而是引导你去登录项自己重新启用。启动时,仍处于「已启用但已陈旧」的服务会被静默重新注册,让后台索引继续工作;而你主动关掉的服务则保持不动。
