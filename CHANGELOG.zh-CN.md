...
- **Spotlight 里直接拨的开关（macOS 26）。** 在 macOS 26 上，运行「设置开关」动作会把一个 SimpleZip 设置以交互式 Spotlight 卡片的形式弹出——卡片里有一个真正的开关，点一下就地翻转、卡片刷新成新状态，不打开 App、不弹窗。它走和语音控制同一份安全开关白名单（同样的三道闸），涉及安全的设置永远不会出现。macOS 25 及更早没有这张卡片——语音开关和点结果跳到设置覆盖那些系统。
