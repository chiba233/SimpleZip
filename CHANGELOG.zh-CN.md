...
- **从 Spotlight 跳到某个设置。** 在 Spotlight 里搜设置名（如「Spotlight 索引」「归档内容缓存」「创建后验证」「显示隐藏文件」「行密度」），点结果就直接落到那一项——SimpleZip 会打开设置、切到对应分页并高亮那一行。各分页里几乎每个选项都覆盖了。只索引设置名、不索引设置值，受同一个 Spotlight 索引开关管。设置 → 自动化 → Spotlight 还会显示大概索引了多少条（归档、文件、设置），并有一个「重建」按钮——清空并重新捐献全部内容，搜索结果不全或过时时用得上。
