...
- **`simplezip` 命令行大幅增强。** `help <命令>` 给单命令详细用法；`--json` 每命令输出一个机器可读结果对象、`--quiet` 只留错误、`--verbose` 透传引擎原始输出；`simplezip doctor` 体检环境（app bundle、7-Zip 引擎、RAR/GPG 后端、符号链接指向）；命令打错给「你是不是想输」建议；`check` 与多文件 `verify` 末尾给 N 通过/M 失败汇总；`create` 新增 `--level 0-9`、`--exclude-junk`、`--reproducible` 与 `--encrypt`——加密口令从 SIMPLEZIP_PASSWORD 环境变量或终端无回显交互读取，绝不接受命令行参数。
