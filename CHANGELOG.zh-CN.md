...
- **修复:从**另一个档案内部**打开的档案 / `.siz`,按「上一级」会一路蹦到物理文件夹,而不是回到父档案里。** 现在打开嵌套档案(或嵌套 `.siz`)会记住父档案位置(档案 + 子目录);在嵌套档案根目录按「上一级」回到那里(如 `.siz` 所在的 `outer.zip/sub/`),逐层后退,而不是退出整条链到外层物理文件夹。直接打开的(顶层)`.siz` / 档案仍和以前一样「上一级」回到其真实所在文件夹;打开任何新的真实档案 / 文件夹会清掉嵌套返回路径。
