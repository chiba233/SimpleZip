...
- **内部：对话框设计语言开始组件化。** 共享对话框组件新增 `TaskDialogShell`（hero + 限高滚动区 + 钉底操作栏一次声明），标准底栏补上 Esc 取消和可选的「禁用取消」态。发布助手、转换格式、创建签名清单三个对话框率先放弃各自手拼的 hero/底栏/padding 副本——其余对话框逐步迁移，让间距与按钮规范只有一份事实来源。
