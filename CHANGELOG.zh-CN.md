...
- **全新的 AI 侧栏只读建议文件夹。** 主窗口侧栏新增 AI 区,含三个全本机、只读的工作区——**需要处理**(最近失败、值得再看一眼的任务)、**发布与校验**(最近的检查 / 测试 / 哈希 / 对比)、**最近的归档**(你最近打开过、已建本机内容索引的归档)。每条都能直接跳进活动中心或打开归档——绝不删除、移动或解压。即使 AI 模型不可用也能用(来自你自己的历史与索引,不靠模型),关掉 AI 助手后整段消失。
