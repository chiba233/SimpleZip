...
- **全新的 AI 侧栏:动态主题文件夹。** 主窗口侧栏新增 AI 区,是一组只读的「主题文件夹」——把按语义属于同一主题的东西聚到一起(共享名字 token、处理过它们的同一个任务、或内容相关的 文件 / 归档内条目 / 任务 / 报告),无论它们物理上散在哪,呈现成一个连贯的虚拟文件夹,带子分组和建议动作(在 Finder 显示、打开、哈希、测试;任何写盘动作一律回原生确认流,绝不自行删除、移动或解压)。它们在后台被发现——哪怕你从没在 app 里打开过那个文件夹——来源是一个 **opt-in、仅白名单、只读** 的文件索引(你授权扫哪些目录;只取元数据——名称、大小、日期——绝不读内容,除非你另外开启更高隐私等级的「预读内容」(额外读取纯文本文档的开头生成经脱敏的结构摘要、并把压缩包内容列进 Spotlight,让主题能按内容匹配,而加密内容、密钥与口令仍绝不读取);系统 / 密钥 / 缓存 / 临时目录永远排除)外加你的活动历史。只有完整、够分量的主题才会被推荐、且数量有上限;端上模型可用时,规则质量门控通过的主题会先留在隐藏竞争池里,只有复核通过的胜者才进入侧栏,所以未复核的半成品不会露出、后台又能探索比展示上限更多的候选,且隐藏池规模与复核预算会随你的展示上限和后台活跃度档位伸缩,在达到 normal 约三分之一、aggressive 约二分之一的显示水位后自动降速,继续补强最有希望的主题而不是一次性把显示区灌满;列表按「主题强度 + 你打开它的频率 + 最近度」软加权排序,点一下绝不会把弱文件夹钉到最上面;每个推荐文件夹都有一个「x」——不感兴趣——会对该主题永久降权、并随时间衰减。打开它就像打开一个真正的文件夹:复用 app 其余部分同一套 返回 / 前进 / 上一级 与地址栏面包屑——双击子分组即进入更深一层——它的行是一个原生列表、可用键盘上下移动,顶部带一个主题头和可编辑的描述(留空时会提示这个文件夹大概关于什么,端上模型之后可能帮你润色——模型只写描述、绝不动成员资格)。助手凭直觉加进来的成员会带一个小小的 AI 角标;右键任一成员可标「我很喜欢」(保留、去角标)或「我不喜欢」(移出并不再自动加入),以此告诉文件夹它的边界在哪;你还可以把一个推荐文件夹标记为长期文件夹,让后台发现不再自动把它刷掉。端上模型可用时,整理由它来做 —— 它会**挑出真正契合主题的文件**(而非只是把固定一批重新摆放)、分组并命名,连文件夹本身的名字也由它起,且全部用你的语言(不可用时退回确定性整理,并如实标注)。而且文件夹会学习:喜欢 / 不喜欢、给分组改名、把文件拖进来、编辑描述,都会反过来影响它下次怎么召回和整理 —— 名字和结构都是 AI 生成的,不是写死的。整段受 AI 助手开关门控,后台索引在你开启并授权一个目录前保持关闭。
...
- **活动中心新增只读 AI 工作台。** AI 助手开启时,任务列表页右侧会出现一个工作台,展示当前列表计数、未读失败提醒,并提供一键建议筛选:失败任务、Finder 解压失败、CLI 校验失败、权限问题、缺失卷和校验和不匹配。自然语言「按描述筛选」输入框现在就在工作台里(从工具栏挪进来了,AI 入口统一成一个),生效的筛选可一键清除;工作台左缘可拖拽调整宽度。它只从本机任务索引派生,只会筛选或定位任务,绝不会重新运行或改动任何东西。
