...
- **XIP 归档(如 Xcode 的 .xip)不再打开后空列表。** 7-Zip 的 `-slt` 输出在 xar 条目上带有 `Type = file` 字段,列表解析器误把它当作归档头的标记,将每个条目都丢弃——导致 XIP 打开后一片空白。解析器现只在 `Type` 值不是条目级别(`file`、`directory`、`symlink`)时才将其视为潜在头标记,XIP 条目由此正确保留。
