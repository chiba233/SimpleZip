...
- **工具栏的上下文按钮开始学习你的习惯。** 工具栏上那两个随选择变化的按钮,现在从与右键菜单同一份完整动作集里取候选(不再是写死的小列表),并按你实际常用的动作重排——无论你是从工具栏还是右键菜单点的——让每种选择下最常用的浮到前面。复选共用同一组建议,某格式不支持的动作根本不会出现,所以没有点不了的灰按钮。设置 → 通用 新增「学习我的工具栏习惯」开关(默认开,全本地,绝不记录文件内容)。
