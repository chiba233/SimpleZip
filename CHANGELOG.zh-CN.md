...
- **本地 AI 报告助手（macOS 26）。** 当 Apple Intelligence 可用时出现五个只读助手：发布检查报告上的「总结风险」、路径安全报告上的「解释等级」、失败任务卡上的「解释失败」、批量体检后的「建议标签」、以及「复制为 Issue」旁的 AI 润色 Issue 草稿。每个助手都吃到报告背后的具体细节——真实的示例条目（被标记的路径、垃圾文件名）、文件数与体积、更完整的命令日志，而不只是界面上展示的汇总计数，这样输出会指出具体的东西而非空泛套话。每个都把结果落进可编辑文本框（可用 Writing Tools 改写）供你审阅、复制——助手只读取与解释，绝不改动任何文件、设置或任务，输入也绝不包含加密归档的内容或任何口令。全程本地推理。可在设置→自动化里关闭；旧系统或模型不可用时整个入口隐藏。
