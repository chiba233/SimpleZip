...
- **从 Spotlight 或 Siri 找出某个文件在哪个压缩包里。** 打开归档时，它里面的非加密文件名会被记在本地，之后你在 Spotlight 里输入一个文件名（或问 Siri、跑「查找含某文件的归档」快捷指令），SimpleZip 就会列出含它的归档——点开结果即在 SimpleZip 里打开那个归档。设置 → 自动化 → 归档内容缓存让你掌控这份记忆：可以关掉（关掉即清空）、设置记住多少个归档、过多少天后忘记、查看当前占用、随时清空。隐私优先——加密归档永不记录（加密条目会被排除，整包加密的归档直接跳过），从别的归档里打开的归档不记录，缓存不进设置备份，恢复出厂时清空。把这些文件名捐献进系统 Spotlight 索引还要遵循 Spotlight 索引开关，关掉任一个都会移除它们。
