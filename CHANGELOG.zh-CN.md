...
- **新增：真·命令行伴侣——终端里的 `simplezip`。** 设置 → 通用 → 命令行工具 一键在 `/usr/local/bin/simplezip` 安装符号链接——目录不可写时弹标准的 macOS 管理员授权窗口（可复制的 `sudo` 命令仍作兜底），链接存在时始终提供卸载按钮。五个子命令跑的是和 app 同一套内置引擎，有真实 stdout 和退出码（0 成功 / 1 有失败或差异 / 2 用法错误），可进脚本和 CI：`check` 测试归档完整性，`compare` 逐条目比较两个归档，`create` 创建归档（格式按输出扩展名推断、自动套用该格式已保存的默认值、绝不覆盖已存在文件），`verify` 校验 SHA256SUMS/校验文件（与 app 内同一套不安全路径防护），`open` 把文件交给 app 打开。每条完成的命令同时记录进活动中心——app 在跑时经跨进程通知实时出现，没在跑则合并写入持久化历史，命令输出可在任务详情里查看。诚实的边界（`simplezip help` 里写明）：暂不支持加密归档（口令绝不该出现在命令行参数里）；CLI 输出仅英文（进程脱离 app bundle 运行时本地化解析不到——实测而非猜测）。
