...
- **最近的发布包与活动中心任务出现在 Spotlight 里。** 发布助手记录的每个发布按版本、日期、产物文件名、SHA-256 前缀索引进 Spotlight（macOS 15+），每个收尾的活动中心任务按标题、操作、来源、状态索引——搜版本号、文件名、或「失败的解压」都能直接找到。只索引这些可公开的元数据；结构指纹与加密归档的内容绝不索引。每次启动重新同步（发布另在每次跑完后、任务另在每条收尾时）；macOS 14 及更早直接跳过。设置→自动化里有开关可关闭索引（并清空已索引内容），想更私密就关掉。
