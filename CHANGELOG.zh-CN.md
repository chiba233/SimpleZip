...
- **设置页统一 hero 外观、并保持沉浸。** 每个设置分区顶部现在都有彩色 hero 头(图标瓦片 + 标题 + 一句内容描述),侧边栏用与活动中心同款的渐变瓦片。窗口定宽、可调高,并全程保持毛玻璃、铺到边的沉浸式效果。

### 修复 (bugfix)

**面向用户**

- **「快捷指令与 Siri」面板仅在 app 有苹果开发者签名时出现。** macOS 26 的 App Intents 执行链要求 validated bundle（带 Team Identifier 的代码签名），所以 ad-hoc / 未签名构建永远无法从快捷指令 App 跑 SimpleZip 动作——系统会拒绝连接、快捷指令报「无法与 App 通信」。与其推广一个注定跑不起来的功能，设置 → 自动化 现在会在当前构建没有 Team Identifier 时整体隐藏「快捷指令与 Siri」一节；构建被正确签名后自动恢复。CLI、URL scheme、Finder 服务、Spotlight 等通道不受影响。
