...
- **活动中心新增只读 AI 工作台。** AI 助手开启时,任务列表页右侧会出现一个工作台,读当前页面、告诉你现在最值得先处理什么、为什么。它把任务列表按严重度和时间智能排序——在跑的、未读失败的排在最前,再按新旧——而不是单纯时间序;总结当前列表,提示未读失败。展开一个失败任务会给出一句话的失败原因解释,旁边有「打开完整 AI 解释」按钮,以及一键的下一步动作,都接到这个任务自己的安全处理器上:打开报告、从失败步继续、重跑、改参重跑、复制诊断。它还提供一键建议筛选(失败任务、Finder 解压失败、CLI 校验失败、权限问题、缺失卷、校验和不匹配)和自然语言「按描述筛选」输入框——都在工作台里、AI 入口统一成一个,可一键清除、左缘可拖拽调宽。一切都从本机任务索引派生、后台预烘焙、只读分析:它只排序、解释、筛选或定位任务,绝不重新运行或改动任何东西。
