...
- **UI：全 app 告别渐变 box,一律纯色平涂。** 设置区改版期间定下的设计铁律（「背景可以柔光,box 绝不渐变」）现在覆盖每个界面：对话框 hero 图标、抽屉瓦片、侧栏行瓦片、自动解压浮窗瓦片、哈希结果页头、冲突对话框警告瓦片、`.siz`/`.szs` 验签大印章,全部改为纯色平涂 + 统一降饱和（侧栏瓦片保持高饱和,两档层次规则）。符号改用 shape+overlay 结构保证绝对居中,宽字形不再看着歪;对话框抽屉内容也跟设置区抽屉一样获得二级缩进。
