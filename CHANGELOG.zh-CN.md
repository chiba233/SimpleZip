...
- **可选的「流式快速解压」(zip 与 tar 归档)。** 整包解压对话框为无密码的 zip 与 tar 家族归档(.zip、.tar、.tar.gz / .tgz、.tar.bz2 / .tbz2、.tar.xz / .txz、.tar.zst)新增「流式快速解压」开关:按归档里的物理顺序读取,而不是靠目录索引到处跳——在网络共享或超大包上快得多。它需手动开启,且仅在无密码时才出现;打开后会隐藏它用不到的密码 / 解密方式选项,万一流式读取失败会自动回退到标准引擎。
