...
- **用一句话切出任务的临时视图（macOS 26）。** 当 Apple Intelligence 可用时，活动中心头部有个 sparkles 按钮，输入一句大白话——「minecraft.zip」「今天失败的」「上周命令行跑的」——本机 AI 就把它变成一个临时分类:同时匹配文件/任务名、时间窗（今天 / 昨天 / 近 7 或 30 天）、状态和来源，而不只是在现成的状态/来源菜单里挑。分类生效时按钮填充紫高亮，点一下即清空、回到全部。AI 只产出过滤条件——由 app 当只读视图应用，模型从不执行动作。AI 助手关闭或不可用时整体隐藏。
