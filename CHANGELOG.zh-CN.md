...
- **本地 AI 报告助手（macOS 26）。** 当 Apple Intelligence 可用时，发布检查报告上多一个「总结风险」按钮、生成大白话风险摘要；失败任务卡上多一个「解释失败」按钮、解释可能原因与下一步该试什么。结果落进可编辑文本框（可用 Writing Tools 改写）供你审阅、复制——助手只读取与解释，绝不改动任何文件、设置或任务，输入也绝不包含加密归档的内容或任何口令。全程本地推理。可在设置→自动化里关闭；旧系统或模型不可用时整个入口隐藏。
