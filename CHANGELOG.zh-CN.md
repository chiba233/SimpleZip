...
- **统一的端上 AI 数据层(更强但依旧隐私的 AI 的底座)。** SimpleZip 正在搭建一套共享、完整单测的底座来决定「怎么把数据喂给苹果端上模型」——定位是「本地事实索引 + 小模型智能组织层」:App 负责把本机事实采集、过滤、脱敏、按预算整理成结构化信封,每个信封都带明确的隐私分级和「省略说明」(哪些数据没给、为什么),外加可复用的证据卡和经校验的来源引用;模型只负责解释、命名、排序、归组、起草。模型返回的每个路径、任务 id、归档条目 id 都会和 App 实际提供的候选校验,任何危险动作一律回到原生确认流。口令、密钥材料、加密归档条目名、密文、解密明文绝不进入这一层——靠契约保证(确定性编码,发给模型的事实可单测、可导出审查)。
