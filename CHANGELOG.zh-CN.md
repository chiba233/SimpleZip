...
- **发布包现在是 App Intents 实体。** 发布账本里每条记录都暴露成可查询的「发布包」实体——版本、日期、格式、SHA-256、文件数、可复现与校验文件标记——这样自动化可以引用某个具体的历史发布，而不是一条裸文件路径。这是下面 Spotlight 索引与实体感知 intent 的地基；只读账本、绝不读归档内容，也不做任何写入或安全判定。
