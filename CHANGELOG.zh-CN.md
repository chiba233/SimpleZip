...
- **一轮系统性安全加固。** 收紧了多处安全面:不可信的归档条目名 / 文件名不再被内置与系统工具当成命令开关;输出与解压目标不再能被引导到你所选文件夹之外(含下载文件名与手填的归档名);签名先写临时文件、成功后才原子替换原文件,取消或失败的签名不再丢失你已有的文件;DMG 解压不再复制指向镜像外的符号链接;导入损坏的设置备份不再崩溃;归档条目名保留精确的前后空格,删除与重命名作用在正确的条目上;可选 AI 助手的进程边界加了大小上限。恢复出厂现在也会清掉 AI 在后台积累的全部数据。
