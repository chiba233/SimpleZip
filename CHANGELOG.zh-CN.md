...
- **用语音开关安全设置——不用打开 App。** 对 Siri 说（或运行「更改设置」快捷指令）就能翻转一个便利类设置——「打开 Spotlight 索引」「关闭自动解压」——SimpleZip 在后台改好、不弹窗；正开着的设置窗口会实时跟着变。只有安全、无破坏性的开关能这样改：涉及安全的设置（删除确认、GPG、预设密码、归档路径安全策略）刻意永远不可语音控制，并用三道闸焊死，精心构造的请求也碰不到它们。
