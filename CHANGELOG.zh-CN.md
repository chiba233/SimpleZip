...
- **SimpleZip 关闭时也会在后台预烘焙 AI 建议了。** 开启「静默后台索引」后,后台助手不再只收集元数据 —— 它会自己跑端上模型,用你的界面语言把真正的建议(文件一句话摘要、网页链接建议……)预先烘焙好,这样你下次打开应用时建议已经在那了,而不是现算。它会一直烘焙到单轮时间预算用尽(整批积压、不是前台那种每轮就几个的上限),下次计划运行时接着上次的继续;首段恰好切在多字节字符中间的大体积中日韩文档现在也能正确生成摘要了。
