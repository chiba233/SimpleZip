...
- **修复:打开 header-encrypted(连文件列表都需要密码)的 `.7z` 时不弹密码框、直接失败。** 之前开档只在「设了预设密码」时重试,没预设(或预设不对)就直接落到报错而不询问。现在会弹密码框并重试(和解压流程同款的弹框-重试),header-encrypted 档案能像普通带密码档案一样打开;在密码框点取消则静默中止、不弹错误。
