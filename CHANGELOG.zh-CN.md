...
- **修复:「创建自解压压缩包」报错「cannot find specified SFX module」。** 自解压(SFX)依赖一个 Windows `.exe` 桩模块(`7zCon.sfx` / `Default.SFX` 等),而 macOS 版的 7-Zip 与 RAR 都不随附它,且产物本身也只能在 Windows 上运行。现在 macOS 上隐藏该选项,不再产生这个错误。(若有「在 Mac 上制作发给 Windows 用户的自解压包」的需求,可再考虑打包这些模块。)
