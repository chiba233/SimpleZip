...
- **修复:文件 / 压缩包列表会出现重复的「名称」列(尤其是从表头菜单切换列之后)。** 一个历史 bug 把「名称」列重复存进了列顺序偏好(`fileColumnOrder` 可能变成 `[name, name, size, …]`),表头就冒出 2~3 个叠在一起的「名称」列。现在启动时、以及每次移动列时都会按 identifier 去重存储,污染被清掉且不会再复发。
