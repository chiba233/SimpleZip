...
- **「快捷指令」动作不再报「Couldn't communicate with a helper application」。** 运行 SimpleZip 的快捷指令动作时,系统会把应用当作 App Intents 助手在后台拉起。应用那套 opt-in 的后台 AI 预烘焙——它会做的事之一是向 LaunchServices 查询每个已索引文件的默认打开应用——可能在这个无界面的后台助手里跑起来,把它占住到错过 App Intents 的连接窗口,于是快捷指令报通信失败。现在后台 AI 工作只在应用真正进入前台之后才启动,后台助手便能保持响应、动作正常执行。
