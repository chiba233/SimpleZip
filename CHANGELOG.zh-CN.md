...
- **新增:文件浏览器里文件夹可原位展开**(Finder 列表视图风格)。目录行带展开箭头,展开时按主列表完全一致的规则(隐藏文件 / 符号链接 / .szs 虚拟过滤)懒加载子级。与此前被回退的版本不同,这次展开的子级是一等公民:多选、右键、重命名、拖动和所有文件操作都对子级生效,自动刷新不丢选区,列表重载后展开的文件夹保持展开并自动同步最新内容。
