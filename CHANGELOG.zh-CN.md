...
- **全新的 AI 侧栏「需要处理」文件夹。** 主窗口侧栏新增 AI 区:点「需要处理」即可看到一份只读、全本机的列表——最近失败、可能值得再看一眼的任务,每条都能直接跳进活动中心。即使 AI 模型不可用它也能用(列表来自你自己的任务历史,不靠模型),它绝不碰你的文件(只「打开 / 定位」,绝不删除、移动或解压),关掉 AI 助手后整段消失。
