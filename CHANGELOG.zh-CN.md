...
- **统一的端上 AI 数据层(更强但依旧隐私的 AI 的底座)。** SimpleZip 正在搭建一套共享、完整单测的底座来决定「怎么把数据喂给苹果端上模型」——定位是「本地事实索引 + 小模型智能组织层」:App 负责把本机事实采集、过滤、脱敏、按预算整理成结构化信封,每个信封都带明确的隐私描述块(端上执行、标准/深度本地上下文强度、确认排除的红线类别)和「省略说明」(哪些数据没给、为什么),外加可复用的证据卡和经校验的来源引用;模型只负责解释、命名、排序、归组、起草。模型返回的每个路径、任务 id、归档条目 id 都会和 App 实际提供的候选校验,任何危险动作一律回到原生确认流。口令、密钥材料、加密归档条目名、密文、解密明文绝不进入这一层——靠契约保证(确定性编码,发给模型的事实可单测、可导出审查)。信封之上还有一层确定性的「组织」基元,模型只做精修:归档画像与角色识别、一套受控语义标签词表(模型只能从中选取并重排,带确定性分数、证据与用户纠错降权)、失败诊断标签与按标签的修复手册、低敏位置上下文、按任务/按归档的 AI 索引、文件类型分类与文件夹画像(角色 + 推荐视角派生)、可保存的查询计划、把已索引对象重新归组的「视角」(发布/源码/失败/签名/清理)、一眼看清归档内部构成的「内部地图」、版本关系标注(同内容/新构建/源码vs二进制/分卷/旧备份/本地化变体)、乱码文件名的确定性候选编码评分(把 raw bytes 用 Shift-JIS / EUC / GBK / Big5 / CP437 / UTF-8 解码并按脚本一致性排序,模型只在候选间重排、绝不发明文件名;加密条目排除)、确定性的下一步动作排序器、一道打扰阈值门(决定 AI 能多吵:安静→状态→内联速览→建议卡,让无害的 Finder 双击保持安静、自动解压绝不被打断)、把模糊搜索重写成可执行本地查询、一个基于 JSON AST 的上下文压缩器(在模型读取前把反复出现的长 key/value 换成短 token 以降低首字延迟、输出时还原——绝不用正则替换、小于 1.5 KB 不压缩)、虚拟 AI 工作区、带硬性默认排除规则(系统/密钥/缓存/开发依赖/临时解密目录)的白名单后台预读作用域,隐式的兴趣/反馈信号、按时段感知的启动目录排序器,全局建议总线契约(每个界面位置发请求→拿回经校验、可忽略的建议卡),确定性的「为什么没有推荐」解释器,以及一套能力分层的引擎协商契约(确定性/端上/高级可选)——让 AI 界面常驻、只有模型增强受门控——创建/解压 Auto-Tune 安全闸(逐字段策略表,让模型预填表单选项,但绝不自动改密码、GPG 材料、加密强度、目标路径或破坏性整理动作,碰加密内容的改动直接丢弃,也绝不覆盖用户已手改的字段),以及确定性的创建/解压建议卡规则引擎(把 dry-run / 预检事实直接变成可点击的卡片——改名输出、查看安全报告、去单根目录、跳过符号链接——只携带稳定规则 id、证据 token 和白名单内动作,无模型也工作,模型发明的动作一律剔除)——每一个都是纯值类型,无模型也能工作,且全部可单测。
