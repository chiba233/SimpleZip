...
- **`simplezip` 命令行与「快捷指令」补齐缺失的归档动词。** 命令行新增 `list`(条目按 类型/大小/名称,或 `--json`)、`inspect`(发布助手的发布包检测、不解压——文件/文件夹数、总大小、macOS 垃圾、空目录、可执行、符号链接与可疑条目路径;有可疑则 exit 1)、`extract`(解进唯一命名文件夹,走与 Finder 自动解压同一条受检安全路径,`--to` 指定目标)和 `hash`(CRC32/MD5/SHA-1/SHA-256/SHA-512,默认 SHA-256,BSD-tag 输出可被 `verify` 读回)。全部支持 `--json`/`--quiet`、记入活动中心,加密归档会弹口令小窗(或读 `SIMPLEZIP_PASSWORD`)——绝不走命令行;shell 补全(zsh/bash/fish)同步覆盖。「快捷指令」新增 **计算文件哈希** 动作,返回十六进制摘要以串联到后续步骤。
