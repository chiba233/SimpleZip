...
- **打开过的归档被记下来，供内容搜索。** 打开归档时，SimpleZip 把它的**非加密**条目名和少量元数据缓存在本地——这是「某个文件在你打开过的哪个压缩包里」搜索的地基。隐私优先：加密条目永不入缓存（加密项的条目名直接排除，整包加密的归档则整条跳过），解压到临时目录的瞬时归档（嵌套 / 内层包）也从不记录。缓存按归档数量封顶、带过期天数、最旧先淘汰；只存在本地偏好里，不进设置备份，恢复出厂时清空。
