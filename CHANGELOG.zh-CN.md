...
- **开发者工具：格式兼容性实验室。** 选一个小文件夹，实验室对真实后端把它在 zip、7z、tar、tar.gz 之间往返——实测什么真正留下来：结构、可执行位、扩展属性、符号链接、时间戳、注释往返（zip）与可复现性（zip/7z）。探针加在临时副本上，原文件夹绝不被改动。按设计藏在开发者工具里。
