...
- **压缩默认值能从你实际的压缩习惯里学。** SimpleZip 现在会记录你创建归档时选的压缩选项——本地、只记非敏感旋钮（压缩率、更新/路径模式、跳过垃圾等），绝不记口令或任何加密内容。在 设置 → 压缩 里编辑某格式的默认值时，多一个「按我最常用的来」按钮，把每一行填成你对这个格式最常选的值（保存前仍可改）。记录默认开，可在同一页关掉。
