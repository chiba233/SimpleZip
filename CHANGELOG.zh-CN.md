...
- **压缩默认值能从你实际的压缩习惯里学。** SimpleZip 现在会记录你创建归档时选的压缩选项——本地、只记非敏感旋钮（压缩率、更新/路径模式、跳过垃圾等），绝不记口令或任何加密内容。在 设置 → 压缩 里编辑某格式的默认值时，多一个「按我最常用的来」按钮，把每一行填成你对这个格式最常选的值（保存前仍可改）。创建对话框里，当你还没为该格式保存默认值时，也会出现一键「套用你常用的设置」，把你最常选的选项直接填进来。记录默认开，可在同一页关掉。
- **解压对话框也能学你常用的解压设置。** SimpleZip 现在会记住你实际选的解压行为开关——本地、只记非敏感项（跳过垃圾、跳过符号链接、解到同名子文件夹、冲突自动重命名、完成后显示、原包移废纸篓），绝不记口令或目标位置。当你的常用选择跟当前所选不同，解压对话框会在「套用预设」旁多出一键 **用你常用的** 按钮把它们填上。只有你亲手确认的对话框才会学——从 Finder 打开时的自动解压绝不学——默认开，可在 设置 → 通用 里关掉。
