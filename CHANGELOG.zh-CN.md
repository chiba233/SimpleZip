...
- **最近的发布包出现在 Spotlight 里。** 发布助手记录的每个发布都会按版本、日期、产物文件名、SHA-256 前缀索引进 Spotlight（macOS 15+）——搜版本号就能直接找到打好的包。只索引这些可公开的元数据；完整路径、结构指纹、归档内容绝不索引。每次启动（及每次新发布后）重新同步；macOS 14 及更早直接跳过。设置→自动化里有开关可关闭索引（并清空已索引内容），想更私密就关掉。
