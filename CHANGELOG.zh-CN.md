...
- **AI 建议落在文件浏览器里,而不是另起一个侧栏。** 0.4.5 的 AI 工作从可见 AI 侧栏收回到现有文件/归档行里:归档行可展开模型生成的一句话定性和动作建议;文件行可显示模型挑出的 SHA-256、推荐打开方式、网页链接、相关活动、安装提示、包内相关文件和只读内联报告;后台缓存的文件组会渲染成额外的可折叠 AI 行,但真实文件行照常保留。写死的抽屉建议已经移除——模型没有产出就保持空抽屉——任何会写盘的动作仍回到 SimpleZip 原生确认/任务流程,只读检查可以在行内显示结果。
...
- **AI 侧栏实验从正式 UI 中收回。** 可复用的虚拟文件夹组件、排序、复核、主题边界学习、反馈和基准测试仍留在内部地基里,但 0.4.5 实际可见的界面是文件/归档抽屉建议,不是独立侧栏入口。
...
- **AI 工作区 token 提取现在能正确处理版本号。** 把文件显示名转换为语义 token 集合的函数原先逐字符扫描、在每个非字母/数字字符处断开,所以「0.4.5」会产生「0」「4」「5」三个单字符片段——全都低于最短 token 长度、全部被丢弃。现在补充一个「按连字符/下划线/空格整段保留」的切割过程,「SimpleZip-0.4.5-macos-arm64.dmg」由此能正确提取出五字符 token「0.4.5」。该修复使发布工作流基准测试中候选池的强 token 覆盖率(专注律)从 80% 提升至 100%。
