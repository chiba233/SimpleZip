...
- **新增：写回失败保留恢复副本。** 改写在原子替换前一刻被叫停时——归档被外部修改、或写后验证失败——做好的工作副本不再随 staging 一起焚毁，而是搬进恢复区（Application Support → SimpleZip → Recovery）。外部改动拦截时这份副本**就是**你基于旧版完成的编辑成果，可直接当另存副本用；验证失败时它是诊断样本。错误信息会给出确切路径，开发者工具新增「写回恢复区」显示入口和一键「清理恢复区」。成功的操作照旧自我清理。
