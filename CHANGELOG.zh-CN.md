...
- **导出报告对话框重新有了取消按钮。** macOS 的确认对话框不会自动补取消，所以「导出报告」菜单（以及临时产物「立即清理」确认）一旦打开就没有反悔出口——现在两处都有显式的取消。
- **失败红点随滚动逐个消除。** 不再是打开分类就一次抹掉整组——现在每张失败卡片在真正滚进屏幕看得见的一瞬间就消掉自己的红点：看到一个消一个，全部滚完红点全消。还在屏幕外的失败照常计数，已看状态重启后保留。
- **`.siz` 现在能用归档工具了。** 测试、批量体检、比较、查重、空间分析、发布检查都接受 `.siz`，跑在它解包出的内层归档上；发布检查的 SHA 和公钥同捆检查仍对 `.siz` 本身——那才是你分发的文件。数据救援把损坏的 `.siz` 当作它本来的 tar 容器整体救出。右键菜单、「归档」菜单与文件夹筛选都把 `.siz` 当成真正的归档。
- **只读归档保留分析菜单。** 打开只读格式（tar、rar、zst、嵌套归档、`.siz` 的内层归档）时，空白处右键菜单不再丢掉「元数据报告」和「复制打开性能报告」——它们什么都不写，理应也在这里。
- **活动中心重启后仍能看报告详情。** 比较结果与发布检查/元数据报告现在随任务历史落盘，重启后「查看报告」照常可用，不必重跑（超大比较会跳过持久化以免历史膨胀）。
- **侧栏失败红点看过即灭。** 红色失败计数现在只数你没看过的失败——打开那个分类就清掉，不用展开详情，且重启后保持清除。
- **活动中心头部不再被挤压。** 有任务运行时，暂停/睡眠/取消控件改为纯图标、页标题钉住，不再塌成竖排；状态菜单旁那个装饰性筛选小图标删掉了。
- **零碎修复。** 元数据报告的条目数不再打印空的 `()`；发布检查报告上限收到 580pt；发布流水线步骤名去掉多余的「步骤:」前缀。
- **报告之间可以跳转了。** 发布检查报告新增「空间分析」按钮，对同一归档直接跑分析；GitHub Issue 的环境表收敛为单一实现，诊断包与所有报告导出共用。
- **开发者工具：格式兼容性实验室。** 选一个小文件夹，实验室对真实后端把它在 zip、7z、tar、tar.gz 之间往返——实测什么真正留下来了：结构、可执行位、扩展属性、符号链接、时间戳、注释往返（zip）与可复现性（zip/7z）。探针加在临时副本上，原文件夹绝不被改动。按设计藏在开发者工具里。
