...
- **SimpleZip 关闭时也会在后台预烘焙 AI 建议了。** 开启「静默后台索引」后,后台助手不再只收集元数据 —— 它会自己跑端上模型,用你的界面语言把真正的建议预先烘焙好,这样你下次打开应用时建议已经在那了,而不是现算。现在覆盖全套:文件一句话摘要与逐文件动作、网页链接建议、磁盘镜像「拖到应用程序」提示、包内「你可能要的文件」与一句「这是什么包」、按文件/类型的工具栏动作排序、文件夹成组与「整理进新文件夹」建议、排队的只读检查(哈希、完整性测试、以及仅当有异常时的一句路径安全 / 发布包检测说明),还有活动中心工作台(失败解释、命名的真实聚集、「现在最该处理什么」、筛选排序)。它会一直烘焙到单轮时间预算用尽(整批积压、不是前台那种每轮就几个的上限),下次计划运行时接着上次的继续;首段恰好切在多字节字符中间的大体积中日韩文档现在也能正确生成摘要了。它读的是与应用同一套 7-Zip/RAR 后端和设置,所以能自己列目录、测试归档。
