...
- **新增:文件浏览的「权限」「属主」列 + 右键 chmod/chown。** 两个可选列(默认关),显示每个条目的 Unix 权限串(`-rw-r--r--`、`drwxr-xr-x`、`lrwxr-xr-x`,含 setuid/setgid/sticky)及其属主用户。可从 显示 → 文件浏览列、表头右键菜单,或 设置 → 视图 里开关。符号链接显示其自身权限(不是目标的)。右键「权限与属主…」打开一个小编辑器,含 读 / 写 / 执行 复选格(属主 / 群组 / 所有人)、属主输入框,以及——选区含文件夹时——「应用到文件夹内所有项目」复选框(用 `chmod -R` / `chown -R` 递归):改自己拥有的文件的权限直接生效(不弹窗);改属主、或改不属于自己的文件的权限,会走一次 macOS 管理员授权弹窗。整个改动作为活动中心里的**文件操作**运行(跟复制 / 移动 / 删除同类,不在「归档操作」下),标题带真实目标值——「更改 a.txt 的权限为 755、属主为 alice…」/「更改 3 个项目的权限为 644…」——并有逐文件「已更改 / 失败」结果(每条「已更改」行显示新的 `rwxr-xr-x (755)` / 属主),不是甩个裸命令。这份 POSIX 元数据只在两列任一启用时才读取,所以默认文件浏览不会为了列个 home 目录就申请 Desktop / Downloads 这类 TCC 受保护目录的访问权限。每格式的默认压缩设置现在也纳入偏好的备份 / 导出、导入(还原语义)与「全部恢复默认」——密码和 GPG 私密字段一如既往不入备份。
