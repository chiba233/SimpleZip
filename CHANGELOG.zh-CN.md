...
- **主窗口支持系统原生标签页（⌘T）和多窗口（⌥⌘N）。** 按 ⌘T 在当前窗口里开新标签、⌥⌘N 开一个独立的新窗口、视图 →「显示标签页栏」（⇧⌘T）可常显标签栏。SimpleZip 已在运行时从 Finder 打开压缩包或文件夹，会在新标签里打开，而不是替换掉你正在看的内容。这是真正的 macOS 窗口标签 —— 只有一个标签时标签栏自动隐藏，「移动标签到新窗口」/「合并所有窗口」都能用。每个标签 / 窗口完全独立（各自的文件夹、选区、撤销历史）。设置、关于、更新窗口仍独立，绝不并进标签组。
- **修复：未运行时从 Finder 打开压缩包来启动 SimpleZip，不弹窗口。** app 没在运行时，双击压缩包只是启动了 app，屏幕上却没有窗口 —— 得点一下 Dock 图标才出现。现在冷启动会可靠地为该文件打开一个窗口。（修复保留了防止 `.siz` / `.szs` 克隆出重复窗口的那层保护。）
- **文件操作支持撤销 / 重做（⌘Z / ⇧⌘Z）。** 移动、粘贴 / 复制、创建副本、重命名、删除 现在都能从「编辑」菜单撤销和重做。撤销删除 = 从废纸篓移回原位；撤销复制 / 粘贴 / 副本 = 删掉新产物（仍可在废纸篓恢复）；移动、重命名原地反向。保守设计 —— 若文件期间被外部改动、或目标位置已被占用，该步会跳过并提示，绝不覆盖任何东西。（正在编辑名称 / 地址栏时，⌘Z 照常撤销打字。）
