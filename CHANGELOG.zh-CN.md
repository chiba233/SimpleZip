...
- **打开 / 解压「条目极多」的归档不再卡死。** 打开大归档、弹出解压对话框、生成解压任务,以及 Finder 自动解压 /右键「用 SimpleZip 解压」,过去在面对由几万个碎文件组成的归档时都会卡住主窗口。现在这几条路径全程保持响应;无密码 ZIP 默认改用内置 7-Zip 引擎解压(macOS 引擎仍作兜底)。

**面向开发者**

- **App target 默认 MainActor 隔离,导致归档列举、解析、ZIP 加密探测和解压后暂存合并都在主线程上静默执行。** 归档的 list/parse 链、ZIP 中央目录加密探测、后端 extract 路径现在都标了 `nonisolated`、跑在主 actor 之外;逐条目的日期解析器改为每次列举只建一次(而非每条目最多建四次——大归档上是几万次 `DateFormatter` 分配);解压对话框复用打开归档时已加载的清单、不再整包重新列举,解压前安全检查也复用这批预载条目;Finder /右键自动解压与一键创建浮窗的逐文件进度,现在和主窗口走同一个 ~80ms 的 `ProgressCoalescer` 合帧,而不再每解压一行就往主 actor 跳一次。
