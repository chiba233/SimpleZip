...
- **Group By（文件浏览器）—— 把文件分成可折叠的区块。** 新增「视图 →文件按…分类」菜单，可选「不分类」（默认）或「按种类」。按种类时文件被切成可折叠区块（图片、文档…），区块默认展开、可手动折叠。和「隐藏文件」组复用同一套 NSOutlineView 基建。（压缩包浏览器的分类是后续 phase。）
  - 同时显示隐藏文件 + 分类时，有个设置决定隐藏文件是融进各种类区块、还是单列一个「隐藏文件」区块（该设置的 UI 随后补，默认融入）。

- **隐藏文件改为收进可折叠分组，不再塞满列表。** 开启「显示隐藏文件」后，OS 隐藏项（点开头文件、Finder 隐藏的文件/文件夹）会收进文件夹末尾一个**默认折叠**的「隐藏文件 (N)」分组，点一下展开。这样文件夹保持清爽，又随时能拿到隐藏项。（开关关闭时行为不变 —— 隐藏文件根本不列出。）
...
- **清理临时文件**（设置 → 运行状态）—— 显示 SimpleZip 的临时产物（打开压缩包的解压目录、签名清单 staging 等）当前占用多大，右侧一个「清理」按钮一键清掉。只动 SimpleZip 自己创建的条目（名字以 `SimpleZip` 开头），不碰系统临时目录里别的东西。
