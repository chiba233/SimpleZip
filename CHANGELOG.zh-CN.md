...
- **新增:文件浏览的「权限」「属主」列。** 两个可选列(默认关),显示每个条目的 Unix 权限串(`-rw-r--r--`、`drwxr-xr-x`、`lrwxr-xr-x`,含 setuid/setgid/sticky)及其属主用户。可从 显示 → 文件浏览列、表头右键菜单,或 设置 → 视图 里开关。符号链接显示其自身权限(不是目标的)。
