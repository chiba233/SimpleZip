...
系统是否把文件交给 SimpleZip 取决于 Launch Services 文件关联。请在设置的“文件关联”里把对应分卷组设为默认，或先用 SimpleZip 的“打开压缩包”选择分卷。
...
SimpleZip 没有直接使用 `NSXPCDecoder`、`NSSecureCoding` 或 `NSKeyedUnarchiver`。如果这类日志只出现在 Xcode 控制台、没有对应崩溃或功能失败，通常是 macOS 系统框架或远程服务输出的安全提示。
...
RARLAB `rar` 是专有/shareware 软件。项目提供本地安装脚本，但不会把 `rar` 二进制提交进 git。公开分发带 RAR 后端的 App 包前，需要确认再分发授权。
