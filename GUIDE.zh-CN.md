...
SimpleZip 是一个面向 macOS 的原生压缩包管理器。它不是把命令行套个壳，而是
把日常压缩软件该有的东西做完整：像 Finder 一样浏览本地文件夹、像 7-Zip /
NanaZip 一样把压缩包当目录浏览、双击直接打开压缩包里的文件、拖拽解压、创建、
哈希、文件关联、**GPG 签名 / 验签**，以及独家的 **`.siz` 单文件签名容器**。

项目主页：[github.com/chiba233/SimpleZip](https://github.com/chiba233/SimpleZip)

## SimpleZip 适合做什么

- 像 Finder 一样浏览本地文件夹，常用文件管理动作齐全（剪切 / 复制 / 粘贴 /
  移动 / 删到废纸篓 / 在 Finder 显示）。
- 像 7-Zip 一样浏览压缩包内部目录树，路径不是平铺的长字符串。
- 不先解压直接打开压缩包里的文档 / 图片 / DMG / `.app` 包。
- 把压缩包里的文件拖到 Finder 或其它 App —— SimpleZip 用 macOS 文件承诺机制，
  放手才解压。
- 创建 ZIP / 7z / RAR / TAR / GZip / TAR.GZ / BZip2 / XZ / DMG。
- 创建 / 校验 `.siz` 单文件 GPG 签名容器。
- 在 Finder 右键直接哈希文件或打包成压缩包，不用先开主窗口。
- 设置默认应用，包括 RAR / DMG / 分卷扩展名（`.001` / `.z01` / `.r00` 等）。
- 多语言 UI（英 / 简中 / 繁中 / 日 / 韩 / 俄 / 德 / 法 / 西 / 泰）。
...
「双击打开」是临时解压 + 交给默认 App，适合查看 / 安装，**不等于在压缩包里
编辑**。要保留改动请另存到正式目录。
