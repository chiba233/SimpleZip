...
可选的 AI 助手(苹果端上 **FoundationModels**,macOS 26+)驱动文件/归档的行内建议、报告「解释一下」、
创建/解压的推荐设置、自适应工具栏、活动中心工作台。它**默认关闭**,且设计上即便开启也绝不会削弱本文档其它
部分的安全保证。

### 在哪里运行

- **完全在你的 Mac 上。** 推理走苹果端上模型;不上传任何东西、无网络调用、无遥测、无账号。
- **在独立进程里。** 模型跑在专门的 XPC 服务 / LaunchAgent 助手里,不在主程序 —— 主二进制不做任何模型推理。
  后台预烘焙只在你开启后、于白名单文件夹、设备空闲且电源允许时才跑。
- **XPC 边界有大小上限。** 配置与每次 pass 的输入 payload 在解码前就按硬上限(1MB / 32MB)拒绝,
  畸形 / 恶意消息无法把助手进程撑爆(OOM)。

### 硬红线(设计上强制)

- **只解释、分类、总结、建议、起草。** **绝不**自行删除、移动、覆盖、解压、签名、放行、绕过安全提示,
  也绝不自行改动任何文件、设置或任务。建议提出的任何可写操作,仍走和手动操作**完全相同**的确认/任务流程与
  路径安全校验;只读检查(哈希 / 测试)仅把结果内联显示。
- **绝不接触机密。** 口令、加密密钥、加密归档条目的内容、解密后的明文,**绝不**送进模型、索引或日志;
  加密归档的条目**名**与密文也排除在 AI 输入之外。
- **不是安全裁决者。** 归档是否可安全解压、签名是否有效,绝不由任何 AI 输出决定 —— 那些仍是规则 / 密码学判定
  (见 `.siz` / `.szs` 与路径安全各节)。AI 可以*描述*结果,绝不据它放行。

### 它能读什么,以及你的掌控

- 索引/预读**仅白名单、只读**:只有你显式添加的文件夹,只读文件元数据 +(单独 opt-in 的)非加密文本内容,
  用来建本地索引和短摘要。非加密的路径/名字/元数据不当机密、可进 prompt;加密内容绝不进。
- 一切**可撤销**:关掉助手会隐藏其入口并停止预烘焙;「清空」会抹掉它在后台积累的全部 —— 文件索引、建议、
  学到的排序、跨表面反馈、待检查队列 —— 绝不动你的真实文件、也绝不动 Spotlight 结果。
...
- **加密源 fail-closed；解压暂存可优雅回落(产品决策)**：解密 `.gpg`/`.siz` **必须**有加密卷,挂不上就**报错**,
  绝不退回把明文写进未加密分区。相对地,把归档**解压到你选的文件夹**时,其中间暂存目录在卷短暂不可用时会回落
  系统临时目录 —— 这是有意的产品决策:既保证 app 不变砖,也因为解压的**最终落点本就是你选的普通(未加密)文件夹**、
  短暂暂存副本并不比你要的结果更暴露。(真正短命的敏感数据 —— 解密产物、以及在归档内**原位**打开文件时的临时副本
  —— 才是留在加密卷上的部分。)
